...
-  SELECT EXACTLY ONE status message using the RANDOM SEED
...
-  SELECT EXACTLY ONE vibrant, visually striking color using the RANDOM SEED
...
-  SELECT EXACTLY ONE show using the RANDOM SEED
...
-  SELECT EXACTLY ONE Coen Brothers film using the RANDOM SEED
...
-  SELECT EXACTLY ONE film using the RANDOM SEED
...
-  SELECT EXACTLY ONE element using the RANDOM SEED
...
-  SELECT EXACTLY ONE type using the RANDOM SEED
...
-  SELECT EXACTLY ONE festival using the RANDOM SEED
...
-  SELECT EXACTLY ONE iconic scene using the RANDOM SEED
...
-  SELECT EXACTLY ONE distortion using the RANDOM SEED
...
-  SELECT EXACTLY ONE typical baby situation using the RANDOM SEED
...
-  SELECT EXACTLY ONE iconic Hanna-Barbera franchise using the RANDOM SEED
...
FILM/SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Hitchcock film using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 15:
...
-  SELECT EXACTLY ONE iconic photograph using the RANDOM SEED
...
-  SELECT EXACTLY ONE idiom using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 18:
  - 0: Breaking the ice
...
FILM/SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Kubrick film using the RANDOM SEED
...
-  SELECT EXACTLY ONE Mandela Effect using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 15:
...
-  SELECT EXACTLY ONE musical using the RANDOM SEED
-  Use the LAST DIGIT of the RANDOM SEED:
  - 0: Grease
  - 1: Footloose
...
-  SELECT EXACTLY ONE farm animal using the RANDOM SEED
-  Use the LAST DIGIT of the RANDOM SEED:
  - 0: horse
  - 1: pig
  - 2: cow
...
-  SELECT EXACTLY ONE factory location using the RANDOM SEED
...
-  SELECT EXACTLY ONE profession using the RANDOM SEED
...
-  SELECT EXACTLY ONE story role using the RANDOM SEED
...
-  SELECT EXACTLY ONE mask style using the RANDOM SEED
...
FILM/SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Ridley Scott epic using the RANDOM SEED
...
SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE scene type using the RANDOM SEED
...
FILM/SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Ron Howard film using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 12:
...
-  SELECT EXACTLY ONE episode-style lesson using the RANDOM SEED
...
-  SELECT EXACTLY ONE comedian's style using the RANDOM SEED
...
-  SELECT EXACTLY ONE streaming platform using the RANDOM SEED
...
FILM/SCENE SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Tarantino film using the RANDOM SEED
...
-  SELECT EXACTLY ONE viral challenge using the RANDOM SEED
...
FILM SELECTION (CRITICAL):
-  SELECT EXACTLY ONE Tim Burton film using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 11:
...
-  If no country is specified, SELECT EXACTLY ONE country using the RANDOM SEED
-  Use the LAST TWO DIGITS of the RANDOM SEED modulo 48:
  ...
  - 3: Mexico (Mariachi suits, Charras outfits)
  - 4: Africa (Zulu attire, African prince/princess costumes)
  - 5: Norway (Bunad)
  ...
  - 12: Mongolia (Colorful ethnic costumes with intricate patterns)
  - 13: Chile (Vibrant polleras, ponchos)
  - 14: Ghana (Kente cloth attire)
  - 15: UAE (Kandura for men, Abaya for women)
  - 16: Argentina (Poncho, bombacha trousers, wide-brimmed hat, pollera skirts)
  - 17: Peru (Poncho, pollera, chullo hat)
  - 18: Brazil (Baiana dress, Bombacha trousers, Camisa de Botão)
  - 19: Colombia (Sombrero Vueltiao, colorful polleras)
  - 20: Canada (Hudson's Bay blanket coats, buckskin dresses, Mackinaw jackets)
  - 21: USA (Colonial attire, Christian folk costumes)
  ...
  - 25: Morocco (Djellaba, Kaftan)
  - 26: Egypt (Kalasiris dress, tunics, head coverings)
  ...
  - 33: China (Hanfu, Zhongshan outfit, Cheongsam, Tang suit)
  ...
  - 37: Ireland (Aran sweater, brat cloak)
  - 38: Romania (Fota, embroidered blouse dress combinations)
  - 39: Denmark (Skjorte, Læderbukser, embroidered blouses with skirts)
  - 40: Britain (Tweed jackets, Barbour jackets, kilts)
  - 41: Switzerland (Dirndl, Appenzeller Tracht, Lederhosen)
  - 42: Scotland (Kilt, Ghillie shirts, tartan patterns)
  - 43: France (Paletot coats, Rococo gowns)
  ...
  - 45: Italy (Regional festival attire, lacework and vibrant southern patterns)
  - 46: Portugal (Peasant dresses, nobility suits, embroidered wraps)
  - 47: Greece (Chiton, Fustanella, himation)
  - Australia: (Akubra hats, Aboriginal ceremonial wear) - use for overflow
...
FINAL RESULT:
The final image must look like a normal photograph of the subject unintentionally striking an over-the-top, ultra-serious fashion model expression - instantly recognizable as a parody of high-fashion posing, confident, intense, and humorously dramatic.`},{name:"ZOO-TOPIA",category:["MOVIES","ANIMAL","ANTHROPOMORPHIC","DISNEY"],message:"Take a picture and transform the subject into a realistic anthropomorphized animal character (Zootopia-style realism). Choose an animal form that naturally fits the subject’s facial structure, body type, and personality. Preserve recognizable facial features, expressions, proportions, hairstyle equivalents (fur patterns), and the subject’s exact clothing translated accurately onto the animal anatomy. The character must stand upright with human-like posture, hands, and gestures while retaining believable animal anatomy, fur detail, and species-specific traits. Photorealistic fur, skin, and fabric interaction with cinematic lighting. The subject must clearly remain the same individual, now reimagined as a believable anthropomorphic animal in a realistic world."}];let m=[...Zi],T=0,Se=-1,Q=navigator.onLine,I=[],Fe=!1,Ne=null,Ht=0;const ki=500,Ai="r1_camera_queue";let Be,Xe,le;const Ri="r1_camera_styles";let fe=[];const Oi="r1_camera_favorites",Si="r1_camera_visible_presets";let E=[],Ie=!1,Y=0,pt="",vt=!0;function Ra(e){lt&&(clearTimeout(lt),lt=null),Ke||(Ke=document.getElementById("style-reveal"),ga=document.getElementById("style-reveal-text")),!(!Ke||!ga)&&(ga.textContent=e,Ke.style.display="block",lt=setTimeout(()=>{Ke&&(Ke.style.display="none"),lt=null},1200))}function $t(){return new Promise((e,t)=>{const a=indexedDB.open($i,Qi);a.onerror=()=>{console.error("Failed to open IndexedDB:",a.error),t(a.error)},a.onsuccess=()=>{$=a.result,console.log("IndexedDB opened successfully"),e($)},a.onupgradeneeded=n=>{$=n.target.result,$.objectStoreNames.contains(Le)||($.createObjectStore(Le,{keyPath:"id"}).createIndex("timestamp","timestamp",{unique:!1}),console.log("Object store created"))}})}async function es(){try{const e=localStorage.getItem("r1_gallery_index");if(!e){console.log("No old gallery data to migrate");return}const t=JSON.parse(e);let a=0;for(const n of t){const i="r1_gallery_"+n,s=localStorage.getItem(i);if(s){const o=JSON.parse(s);for(const r of o)await Ni(r),a++;localStorage.removeItem(i)}}localStorage.removeItem("r1_gallery_index"),console.log(`Migration complete: ${a} images migrated to IndexedDB`),await Qt()}catch(e){console.error("Migration failed:",e)}}async function Qt(){try{$||await $t(),L=[];const a=$.transaction([Le],"readonly").objectStore(Le).getAll();return new Promise((n,i)=>{a.onsuccess=()=>{L=a.result||[];const s=localStorage.getItem(vi);s&&(gt=s),L.sort((o,r)=>r.timestamp-o.timestamp),console.log(`Gallery loaded: ${L.length} images`),n()},a.onerror=()=>{console.error("Failed to load gallery:",a.error),L=[],i(a.error)}})}catch(e){console.error("Error loading gallery:",e),L=[]}}async function Ni(e){try{$||await $t();const n=$.transaction([Le],"readwrite").objectStore(Le).add(e);return new Promise((i,s)=>{n.onsuccess=()=>{console.log("Image saved to IndexedDB"),i()},n.onerror=()=>{console.error("Failed to save image:",n.error),s(n.error)}})}catch(t){console.error("Error saving image:",t)}}async function Ci(e){try{$||await $t();const n=$.transaction([Le],"readwrite").objectStore(Le).delete(e);return new Promise((i,s)=>{n.onsuccess=()=>{console.log("Image deleted from IndexedDB"),i()},n.onerror=()=>{console.error("Failed to delete image:",n.error),s(n.error)}})}catch(t){console.error("Error deleting image:",t)}}async function Li(e){const t={id:Date.now().toString()+"-"+Math.random().toString(36).substr(2,9),imageBase64:e,timestamp:Date.now()};L.unshift(t),await Ni(t),console.log(`Image added. Total: ${L.length}`)}function ts(e){return!dt&&!ht?e:e.filter(t=>{const a=new Date(t.timestamp);a.setHours(0,0,0,0);const n=a.getTime();let i=!0,s=!0;if(dt){const o=new Date(dt).getTime();i=n>=o}if(ht){const o=new Date(ht).getTime();s=n<=o}return i&&s})}function as(e){const t=[...e];return gt==="newest"?t.sort((a,n)=>n.timestamp-a.timestamp):t.sort((a,n)=>a.timestamp-n.timestamp),t}function Oa(){let e=ts(L);return as(e)}async function ne(){xe(),Wi(),R&&R.style.display==="block"&&na(),await Qt();const e=document.getElementById("gallery-modal"),t=document.getElementById("gallery-grid"),a=document.getElementById("gallery-pagination"),n=document.getElementById("page-info"),i=document.getElementById("prev-page"),s=document.getElementById("next-page"),o=document.getElementById("gallery-count");o&&(o.textContent=L.length);const r=document.getElementById("gallery-sort-order");r&&(r.value=gt);const l=Oa();if(l.length===0)t.innerHTML='<div class="gallery-empty">No photos match the selected filter.</div>',a.style.display="none";else{const c=Math.ceil(l.length/Dt);K=Math.min(K,c);const u=(K-1)*Dt,f=Math.min(u+Dt,l.length),v=l.slice(u,f),w=document.createDocumentFragment();v.forEach(O=>{const B=document.createElement("div");if(B.className="gallery-item",Ce&&S.has(O.id)&&B.classList.add("selected"),Ce){const V=document.createElement("input");V.type="checkbox",V.className="gallery-item-checkbox",V.checked=S.has(O.id),V.addEventListener("click",ve=>{ve.stopPropagation(),ni(O.id)}),B.appendChild(V)}const ie=document.createElement("img");ie.src=O.imageBase64,ie.alt="Gallery image",ie.loading="lazy",B.appendChild(ie),B.onclick=()=>{if(Ce)ni(O.id),ne();else{const V=L.findIndex(ve=>ve.id===O.id);os(V)}},w.appendChild(B)}),t.innerHTML="",t.appendChild(w),c>1?(a.style.display="flex",n.textContent=`Page ${K} of ${c}`,i.disabled=K===1,s.disabled=K===c):a.style.display="none"}e.style.display="flex"}async function ns(){document.getElementById("gallery-modal").style.display="none",K=1,await qi()}function is(){const e=Oa(),t=Math.ceil(e.length/Dt);K<t&&(K++,ne())}function ss(){K>1&&(K--,ne())}function pa(){K=1,ne()}function ai(e,t){const a=e==="start"?"gallery-start-date-btn":"gallery-end-date-btn",n=document.getElementById(a);if(!n)return;const i=n.querySelector(".date-button-text");if(i)if(t){const o=new Date(t+"T00:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"});i.textContent=o,n.classList.add("has-date")}else i.textContent=e==="start"?"Start":"End",n.classList.remove("has-date")}function os(e){if(e<0||e>=L.length)return;J=e;const t=L[e],a=document.getElementById("image-viewer"),n=document.getElementById("viewer-image"),i=document.getElementById("viewer-prompt");n.src=t.imageBase64,n.style.transform="scale(1) translate(0, 0)",oe=1,i.value="",a.style.display="flex",document.getElementById("gallery-modal").style.display="none"}function rs(){document.getElementById("image-viewer").style.display="none",J=-1,oe=1;const e=document.getElementById("gallery-modal");e.style.display="flex",ne()}async function ls(){if(!(J<0||J>=L.length)&&confirm("Delete this image from gallery?")){const e=L[J];await Ci(e.id),L.splice(J,1),document.getElementById("image-viewer").style.display="none",J=-1,oe=1,ne()}}function cs(){const e=document.getElementById("preset-selector");kt();const t=document.getElementById("preset-count");t&&(t.textContent=m.length),e.style.display="flex",he=!0,x=0,Ve(),setTimeout(()=>{const a=document.getElementById("preset-list");a&&Ta>0&&(a.scrollTop=Ta)},50)}function Gt(){const e=document.getElementById("preset-list");e&&(Ta=e.scrollTop),document.getElementById("preset-selector").style.display="none",Bt="",Ge="",document.getElementById("preset-filter").value="",he=!1,x=0;const t=document.getElementById("preset-selector-category-hint");t&&(t.style.display="none"),_t=!1}function ds(){if(!he)return;const e=document.getElementById("preset-list");!e||e.querySelectorAll(".preset-item").length===0||(x=Math.max(0,x-1),Ve())}function hs(){if(!he)return;const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");t.length!==0&&(x=Math.min(t.length-1,x+1),Ve())}function Ve(){const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("preset-selected")}),x>=0&&x<t.length)){const a=t[x];a.classList.add("preset-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=a.querySelector(".preset-name").textContent,i=m.find(o=>o.name===n),s=document.getElementById("preset-selector-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",Ge===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),Ge===o?Ge="":Ge=o,x=0,kt()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function us(){if(!P)return;const e=document.getElementById("settings-submenu");!e||e.querySelectorAll(".menu-section-button").length===0||(ee=Math.max(0,ee-1),Sa())}function ms(){if(!P)return;const e=document.getElementById("settings-submenu");if(!e)return;const t=e.querySelectorAll(".menu-section-button");t.length!==0&&(ee=Math.min(t.length-1,ee+1),Sa())}function Sa(){const e=document.getElementById("settings-submenu");if(!e)return;const t=e.querySelectorAll(".menu-section-button");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("menu-selected")}),ee>=0&&ee<t.length)){const a=t[ee];a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"})}}function gs(){const e=document.getElementById("resolution-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelectorAll(".resolution-item");t.length!==0&&(te=(te-1+t.length)%t.length,Na(t))}function ps(){const e=document.getElementById("resolution-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelectorAll(".resolution-item");t.length!==0&&(te=(te+1)%t.length,Na(t))}function Na(e){if(e.forEach(t=>t.classList.remove("menu-selected")),te>=0&&te<e.length){const t=e[te];t.classList.add("menu-selected"),t.scrollIntoView({behavior:"smooth",block:"nearest"})}}function fs(){const e=document.getElementById("burst-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ys(){const e=document.getElementById("burst-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function bs(){const e=document.getElementById("timer-settings-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Ts(){const e=document.getElementById("timer-settings-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Es(){const e=document.getElementById("master-prompt-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ws(){const e=document.getElementById("master-prompt-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Is(){const e=document.getElementById("motion-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function vs(){const e=document.getElementById("motion-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Mi(){if(!nt)return;const e=document.getElementById("preset-builder-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".preset-builder-form");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ji(){if(!nt)return;const e=document.getElementById("preset-builder-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".preset-builder-form");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function ks(){const e=document.getElementById("gallery-modal");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".gallery-scroll-container");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function As(){const e=document.getElementById("gallery-modal");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".gallery-scroll-container");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Rs(){const e=document.getElementById("image-viewer");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".viewer-controls");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Os(){const e=document.getElementById("image-viewer");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".viewer-controls");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Ss(){const e=document.getElementById("style-editor");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".style-editor-body");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Ns(){const e=document.getElementById("style-editor");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".style-editor-body");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Cs(){const e=document.getElementById("queue-manager");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".queue-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Ls(){const e=document.getElementById("queue-manager");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".queue-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Ms(){if(!he)return;const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");if(t.length===0||x>=t.length)return;const a=t[x];a&&a.click()}function kt(){const e=document.getElementById("preset-list");e.innerHTML="";const a=Ks().filter(r=>{if(Bt){const l=Bt.toLowerCase(),c=r.category&&r.category.some(f=>f.toLowerCase().includes(l));if(!(r.name.toLowerCase().includes(l)||r.message.toLowerCase().includes(l)||c))return!1}return Ge?r.category&&r.category.includes(Ge):!0}).sort((r,l)=>r.name.localeCompare(l.name)),n=a.filter(r=>at(r.name)),i=a.filter(r=>!at(r.name)),s=[...n,...i];if(s.length===0){e.innerHTML='<div class="preset-empty">No presets found</div>';return}s.forEach(r=>{const l=document.createElement("div");l.className="preset-item";const c=document.createElement("div");c.className="preset-name",c.textContent=r.name;const u=document.createElement("div");u.className="preset-description preset-description-hidden",u.textContent=r.message,l.appendChild(c),l.appendChild(u),l.onclick=()=>{u.classList.contains("preset-description-hidden")?u.classList.remove("preset-description-hidden"):js(r)},e.appendChild(l)});const o=document.getElementById("preset-count");o&&(o.textContent=s.length)}async function js(e){if(_t){const a=Te.findIndex(n=>n.name===e.name);a>-1?Te.splice(a,1):Te.push(e),xi();return}if(window.batchProcessingActive){window.batchProcessingActive=!1;const a=window.batchImagesToProcess;window.batchImagesToProcess=null,Gt();const i=document.getElementById("preset-selector").querySelector(".preset-selector-header h3");i.textContent="Select Preset",await Bs(e,a);return}const t=document.getElementById("viewer-prompt");t.value=e.message,Gt()}function xs(){if(J<0||J>=L.length){alert("No image selected");return}const t=document.getElementById("viewer-prompt").value.trim();if(!t){alert("Please enter a prompt or load a preset");return}const a=L[J];typeof PluginMessageHandler<"u"?(PluginMessageHandler.postMessage(JSON.stringify({message:sa(t,"Custom Prompt"),pluginId:"com.r1.pixelart",imageBase64:a.imageBase64})),alert("Magic transform submitted! You can submit again with a different prompt.")):alert("Magic transform sent: "+t.substring(0,50)+"...")}function Ft(){Ce=!Ce;const e=document.getElementById("batch-mode-toggle"),t=document.getElementById("batch-controls"),a=document.getElementById("batch-action-bar");Ce?(e.textContent="Done",e.classList.add("active"),t.style.display="flex",a.style.display="flex",S.clear(),Zt(),ne()):(e.textContent="Select",e.classList.remove("active"),t.style.display="none",a.style.display="none",S.clear(),ne())}function Zt(){const e=document.getElementById("batch-selected-count"),t=document.getElementById("batch-apply-preset"),a=document.getElementById("batch-delete");e.textContent=`${S.size} selected`,t.disabled=S.size===0,a&&(a.disabled=S.size===0)}function Ds(){const e=Oa();S.clear(),e.forEach(t=>S.add(t.id)),Zt(),ne()}function Ps(){S.clear(),Zt(),ne()}function ni(e){S.has(e)?S.delete(e):S.add(e),Zt()}async function Us(){if(S.size===0)return;const e=document.getElementById("preset-selector"),t=e.querySelector(".preset-selector-header h3");t.textContent=`Select Preset (${S.size} images)`;const a=Array.from(S);window.batchProcessingActive=!0,window.batchImagesToProcess=a,kt(),e.style.display="flex",he=!0,x=0,Ve()}async function Bs(e,t){const a=t||Array.from(S),n=a.length,i=document.createElement("div");i.className="batch-progress-overlay",i.innerHTML=`
  ...
  `,document.body.appendChild(i);let s=0;for(const o of a){const r=L.find(l=>l.id===o);if(r)try{const l=sa(e.message,e.name);typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({message:l,pluginId:"com.r1.pixelart",imageBase64:r.imageBase64})),s++,document.getElementById("batch-current").textContent=s,document.getElementById("batch-progress-fill").style.width=`${s/n*100}%`,await new Promise(c=>setTimeout(c,2e3))}catch(l){console.error(`Failed to process image ${o}:`,l)}}document.body.removeChild(i),Ce=!1,S.clear(),Ft(),alert(`Batch processing complete! ${s} of ${n} images submitted.`)}async function Hs(){if(S.size===0)return;const e=S.size;if(!confirm(`Are you sure you want to delete ${e} selected image${e>1?"s":""}? This cannot be undone.`))return;const a=Array.from(S),n=document.createElement("div");n.className="batch-progress-overlay",n.innerHTML=`
    <div class="batch-progress-text">Deleting <span id="batch-current">0</span> / ${e}</div>
    <div class="batch-progress-bar">
      <div class="batch-progress-fill" id="batch-progress-fill" style="width: 0%"></div>
    </div>
  `,document.body.appendChild(n);let i=0;for(const s of a)try{await Ci(s),i++,document.getElementById("batch-current").textContent=i,document.getElementById("batch-progress-fill").style.width=`${i/e*100}%`}catch(o){console.error(`Failed to delete image ${s}:`,o)}document.body.removeChild(n),Ce=!1,S.clear(),await Qt(),Ft(),alert(`${i} of ${e} image${i>1?"s":""} deleted successfully.`)}function Gs(e){Ut=e,Te=[],_t=!0;const t=document.getElementById("preset-selector"),a=t.querySelector(".preset-selector-header h3");a.innerHTML='Select Presets (Multi-Select) <span id="multi-preset-count" style="font-size: 12px; color: #666;">(0 selected)</span>';let n=document.getElementById("multi-preset-controls");if(!n){n=document.createElement("div"),n.id="multi-preset-controls",n.style.cssText="padding: 8px; background: #f5f5f5; border-bottom: 1px solid #ddd; display: flex; gap: 8px; justify-content: space-between;",n.innerHTML=`
    ...
    `;const i=document.getElementById("preset-filter");i.parentNode.insertBefore(n,i.nextSibling)}n.style.display="flex",kt(),xi(),t.style.display="flex",he=!0,x=0,Ve(),document.getElementById("multi-preset-apply").onclick=Fs,document.getElementById("multi-preset-cancel").onclick=Di}function xi(){document.getElementById("preset-list").querySelectorAll(".preset-item").forEach(n=>{const i=n.querySelector(".preset-name").textContent;Te.some(o=>o.name===i)?(n.style.background="#e8f5e9",n.style.border="2px solid #4CAF50"):(n.style.background="",n.style.border="")});const a=document.getElementById("multi-preset-count");a&&(a.textContent=`(${Te.length} selected)`)}function Di(){_t=!1,Ut=null,Te=[];const e=document.getElementById("multi-preset-controls");e&&(e.style.display="none");const t=document.querySelector(".preset-selector-header h3");t.textContent="Select Preset",Gt()}async function Fs(){if(Te.length===0){alert("Please select at least one preset");return}if(!Ut){alert("No image selected");return}const e=L.find(i=>i.id===Ut);if(!e){alert("Image not found");return}const t=[...Te];Di();const a=document.createElement("div");a.className="batch-progress-overlay",a.innerHTML=`
    <div class="batch-progress-text">Applying preset <span id="batch-current">0</span> / ${t.length}</div>
    <div class="batch-progress-bar">
      <div class="batch-progress-fill" id="batch-progress-fill" style="width: 0%"></div>
    </div>
  `,document.body.appendChild(a);let n=0;for(const i of t)try{const s=sa(i.message,i.name);typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({message:s,pluginId:"com.r1.pixelart",imageBase64:e.imageBase64})),n++,document.getElementById("batch-current").textContent=n,document.getElementById("batch-progress-fill").style.width=`${n/t.length*100}%`,await new Promise(o=>setTimeout(o,2e3))}catch(s){console.error(`Failed to apply preset ${i.name}:`,s)}document.body.removeChild(a),alert(`${n} preset${n>1?"s":""} applied successfully!`)}function Ys(){const e=document.getElementById("viewer-image"),t=document.querySelector(".image-viewer-container");let a=0,n=0,i=0,s=0,o=!1;t.addEventListener("touchstart",r=>{r.touches.length===2?(r.preventDefault(),Lt=!0,Zn=ii(r.touches[0],r.touches[1]),ei=oe):r.touches.length===1&&oe>1&&(o=!0,i=r.touches[0].clientX-a,s=r.touches[0].clientY-n)},{passive:!1}),t.addEventListener("touchmove",r=>{if(Lt&&r.touches.length===2){r.preventDefault();const c=ii(r.touches[0],r.touches[1])/Zn;oe=Math.max(1,Math.min(ei*c,5)),e.style.transform=`scale(${oe}) translate(${a}px, ${n}px)`}else o&&r.touches.length===1&&oe>1&&(r.preventDefault(),a=r.touches[0].clientX-i,n=r.touches[0].clientY-s,e.style.transform=`scale(${oe}) translate(${a}px, ${n}px)`)},{passive:!1}),t.addEventListener("touchend",r=>{r.touches.length<2&&(Lt=!1),r.touches.length===0&&(o=!1,oe===1&&(a=0,n=0,e.style.transform="scale(1) translate(0, 0)"))}),t.addEventListener("touchcancel",()=>{Lt=!1,o=!1})}function ii(e,t){const a=e.clientX-t.clientX,n=e.clientY-t.clientY;return Math.sqrt(a*a+n*n)}function it(){if(!N)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0)return;t.forEach(n=>{n.classList.remove("menu-selected")}),U=Math.max(0,Math.min(U,t.length-1));const a=t[U];if(a){a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=parseInt(a.dataset.index),i=m[n],s=document.getElementById("menu-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",pe===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),pe===o?pe="":pe=o,U=0,Ee()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function zs(){if(!N||!je)return;const e=document.getElementById("menu-styles-list");!e||e.querySelectorAll(".style-item").length===0||(U=Math.max(0,U-1),it())}function Vs(){if(!N||!je)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");t.length!==0&&(U=Math.min(t.length-1,U+1),it())}function Ws(){if(!N||!je)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0||U>=t.length)return;const a=t[U];if(a&&a.querySelector(".style-name")){const s=Rt()[U];if(s){const o=m.findIndex(r=>r===s);o!==-1&&(T=o,z(),xa())}}}function qs(){const e=localStorage.getItem(Ri);if(e)try{const i=JSON.parse(e).filter(s=>s.internal===!1);m.push(...i)}catch(n){console.error("Error loading styles:",n)}const t=localStorage.getItem(Oi);if(t)try{fe=JSON.parse(t),Array.isArray(fe)||(fe=[])}catch(n){console.error("Error parsing favorite styles:",n),fe=[]}eo(),Qs(),Xs();const a=localStorage.getItem(Si);if(a)try{E=JSON.parse(a),Array.isArray(E)||(E=[])}catch(n){console.error("Error parsing visible presets:",n),E=[]}E.length===0&&(E=m.map(n=>n.name),Me())}function Me(){try{localStorage.setItem(Si,JSON.stringify(E))}catch(e){console.error("Error saving visible presets:",e)}}function Ks(){return m.filter(e=>E.includes(e.name))}function Js(e){try{localStorage.setItem(hi,e.toString())}catch(t){console.error("Error saving resolution:",t)}}function Xs(){const e=localStorage.getItem(ui);e!==null&&($e=parseInt(e))}function _s(){localStorage.setItem(ui,$e.toString())}function At(){h&&setTimeout(()=>{const e=ka[$e];switch(h.style.filter="",e.value){case"daylight":h.style.filter="brightness(1.05) saturate(1.1)";break;case"cloudy":h.style.filter="brightness(1.1) saturate(0.95) sepia(0.05)";break;case"tungsten":h.style.filter="brightness(0.95) saturate(1.15) hue-rotate(-10deg)";break;case"fluorescent":h.style.filter="brightness(1.02) saturate(1.05) hue-rotate(5deg)";break;case"candlelight":h.style.filter="brightness(0.85) saturate(1.3) sepia(0.15) hue-rotate(-15deg)";break;case"moonlight":h.style.filter="brightness(0.7) saturate(0.8) hue-rotate(15deg) contrast(1.1)";break;case"auto":default:h.style.filter="";break}},50)}function Pi(e,t,a){const n=ka[$e];if(n.value==="auto")return;const i=e.getImageData(0,0,t,a),s=i.data;let o=1,r=1,l=0,c=1;switch(n.value){case"daylight":o=1.05,r=1.1,l=5;break;case"cloudy":o=1.1,r=.95,l=10;break;case"tungsten":o=.95,r=1.15,l=-20;break;case"fluorescent":o=1.02,r=1.05,l=-10;break;case"candlelight":o=.85,r=1.3,l=25,c=.95;break;case"moonlight":o=.7,r=.8,l=-15,c=1.1;break}for(let u=0;u<s.length;u+=4){let f=s[u],v=s[u+1],w=s[u+2];l>0?(f=Math.min(255,f+l),v=Math.min(255,v+l*.5)):l<0&&(w=Math.min(255,w-l)),f*=o,v*=o,w*=o;const O=.2989*f+.587*v+.114*w;f=O+r*(f-O),v=O+r*(v-O),w=O+r*(w-O),f=((f/255-.5)*c+.5)*255,v=((v/255-.5)*c+.5)*255,w=((w/255-.5)*c+.5)*255,s[u]=Math.max(0,Math.min(255,f)),s[u+1]=Math.max(0,Math.min(255,v)),s[u+2]=Math.max(0,Math.min(255,w))}e.putImageData(i,0,0)}function $s(){document.getElementById("settings-submenu").style.display="none";const e=document.getElementById("white-balance-submenu"),t=document.getElementById("white-balance-list");t.innerHTML="",ka.forEach((a,n)=>{const i=document.createElement("div");i.className="resolution-item",n===$e&&i.classList.add("active");const s=document.createElement("span");s.className="resolution-name",s.textContent=a.name,i.appendChild(s),i.onclick=()=>{$e=n,_s(),document.getElementById("current-white-balance-display").textContent=a.name,k&&At(),Ui()},t.appendChild(i)}),e.style.display="flex"}function Ui(){document.getElementById("white-balance-submenu").style.display="none",document.getElementById("settings-submenu").style.display="flex"}function Qs(){try{const e=localStorage.getItem(hi);if(e!==null){const t=parseInt(e,10);t>=0&&t<Et.length&&(ye=t)}}catch(e){console.error("Error loading resolution:",e)}}function ft(){const t=m.filter(i=>E.includes(i.name)).slice().sort((i,s)=>i.name.localeCompare(s.name)),a=t.filter(i=>at(i.name)),n=t.filter(i=>!at(i.name));return{favorites:a,regular:n}}function Rt(){const{favorites:e,regular:t}=ft(),a=e.filter(i=>E.includes(i.name)),n=t.filter(i=>E.includes(i.name));return[...a,...n]}function Bi(){const e=Rt(),t=m[T];return e.findIndex(a=>a===t)}function Hi(e){const a=Rt()[e];return m.findIndex(n=>n===a)}function ea(){try{const e=m.filter(t=>t.internal===!1);localStorage.setItem(Ri,JSON.stringify(e))}catch(e){console.error("Error saving styles:",e)}}function Zs(e){const t=fe.indexOf(e);t>-1?fe.splice(t,1):fe.push(e),localStorage.setItem(Oi,JSON.stringify(fe));const a=document.querySelector(".styles-menu-scroll-container"),n=a?a.scrollTop:0;Ee(),a&&(a.scrollTop=n)}function eo(){const e=localStorage.getItem(pi);if(e!==null)try{const t=parseInt(e,10);t>=0&&t<m.length&&(T=t)}catch(t){console.error("Error loading last used style:",t)}}function at(e){return fe.includes(e)}function Gi(){const e=m.filter(n=>!n.internal&&at(n.name));if(e.length>0){const n=e[Math.floor(Math.random()*e.length)];return m.findIndex(i=>i===n)}const t=m.filter(n=>!n.internal);if(t.length===0)return 0;const a=t[Math.floor(Math.random()*t.length)];return m.findIndex(n=>n===a)}function to(){ce=!ce;const e=document.getElementById("motion-toggle");if(ce)e.classList.add("active"),e.title="Motion Detection: ON",showStatus("Motion Detection enabled - Press side button to start",3e3);else{e.classList.remove("active"),e.title="Motion Detection: OFF",Ma(),ct&&(clearInterval(ct),ct=null);const t=document.getElementById("timer-countdown");t&&(t.style.display="none",t.classList.remove("countdown-fade-in","countdown-fade-out"));const a=document.getElementById("camera-button");a&&A.length>1&&(a.style.display="flex"),showStatus("Motion Detection OFF",2e3)}}function ao(){for(let e in mt)if(mt[e].seconds===tt)return parseInt(e);return 1}function no(){document.getElementById("settings-submenu").style.display="none",document.getElementById("motion-submenu").style.display="flex",Xt=!0,P=!1}function io(){document.getElementById("motion-submenu").style.display="none",Xt=!1,ue()}function so(){document.getElementById("settings-submenu").style.display="none",document.getElementById("visible-presets-submenu").style.display="flex",N=!1,Ie=!0,vt=!0,P=!1,Y=0,pt="",document.getElementById("visible-presets-filter").value="",_e(),Yt()}function oo(){document.getElementById("visible-presets-submenu").style.display="none",Ie=!1,vt=!1,Y=0,pt="",He="";const e=document.getElementById("visible-presets-category-hint");e&&(e.style.display="none"),ue()}function Fi(){document.getElementById("settings-submenu").style.display="none",document.getElementById("preset-builder-submenu").style.display="flex",N=!1,P=!1,nt=!0,ta()}function Ca(){document.getElementById("preset-builder-submenu").style.display="none",nt=!1,re=-1;const e=document.getElementById("preset-builder-delete");e&&(e.style.display="none"),ue()}function ta(){re=-1,document.getElementById("preset-builder-name").value="",document.getElementById("preset-builder-category").value="",document.getElementById("preset-builder-template").value="",document.getElementById("preset-builder-prompt").value="";const e=document.getElementById("preset-builder-delete");e&&(e.style.display="none");const t=document.getElementById("preset-builder-clear");t&&(t.style.display="flex")}function ro(e){const t=m[e];Fi(),re=e,setTimeout(()=>{const a=document.getElementById("preset-builder-name"),n=document.getElementById("preset-builder-category"),i=document.getElementById("preset-builder-prompt"),s=document.getElementById("preset-builder-template"),o=document.getElementById("preset-builder-delete"),r=document.getElementById("preset-builder-clear");a&&(a.value=t.name),n&&(n.value=t.category?t.category.join(", "):""),i&&(i.value=t.message),s&&(s.value=""),o&&(o.style.display="flex"),r&&(r.style.display="none")},100)}function lo(){const e=document.getElementById("preset-builder-template"),t=document.getElementById("preset-builder-prompt"),a=e.value;a&&ti[a]!==void 0&&(t.value=ti[a])}function co(){const e=new Set;return m.forEach(t=>{t.category&&Array.isArray(t.category)&&t.category.forEach(a=>{e.add(a.toUpperCase())})}),Array.from(e).sort()}function ho(){const e=document.getElementById("preset-builder-name").value.trim(),t=document.getElementById("preset-builder-category").value.trim(),a=document.getElementById("preset-builder-prompt").value.trim();if(!e){alert("Please enter a preset name");return}if(!a){alert("Please enter a prompt");return}const n=t?t.split(",").map(i=>i.trim().toUpperCase()).filter(i=>i.length>0):["CUSTOM"];if(re>=0){const i=m[re].name;if(m[re]={name:e.toUpperCase(),category:n,message:a,internal:!1},i!==e.toUpperCase()){const s=E.indexOf(i);s>-1&&(E[s]=e.toUpperCase())}}else{const i=m.findIndex(o=>o.name.toUpperCase()===e.toUpperCase());if(i!==-1){if(!confirm(`A preset named "${e}" already exists. Do you want to overwrite it?`))return;m.splice(i,1)}const s={name:e.toUpperCase(),category:n,message:a,internal:!1};m.push(s),E.includes(s.name)||E.push(s.name)}Me(),ea(),alert(re>=0?`Preset "${e}" updated!`:`Preset "${e}" saved successfully!`),ta(),Ca(),N&&Ee()}function uo(){if(re<0){alert("No preset selected for deletion");return}const e=m[re];if(e.internal!==!1){alert("Cannot delete built-in presets");return}if(!confirm(`Delete preset "${e.name}"? This cannot be undone.`))return;m.splice(re,1);const t=E.indexOf(e.name);t>-1&&(E.splice(t,1),Me()),T>=m.length&&(T=m.length-1),ea(),alert(`Preset "${e.name}" deleted successfully!`),ta(),Ca(),N&&Ee()}function _e(){const e=document.getElementById("visible-presets-list");e.innerHTML="";const n=m.filter(o=>!o.internal).filter(o=>{if(pt){const r=pt.toLowerCase(),l=o.category&&o.category.some(u=>u.toLowerCase().includes(r));if(!(o.name.toLowerCase().includes(r)||l))return!1}return He?o.category&&o.category.includes(He):!0}).sort((o,r)=>o.name.localeCompare(r.name)),i=document.createDocumentFragment();n.forEach(o=>{const r=document.createElement("div");r.className="style-item",r.dataset.presetName=o.name;const l=document.createElement("input");l.type="checkbox",l.className="master-prompt-checkbox",l.checked=E.includes(o.name),l.style.marginRight="3vw";const c=document.createElement("span");c.className="style-name",c.textContent=o.name,r.appendChild(l),r.appendChild(c),l.onclick=u=>{u.stopPropagation(),si(o.name,l.checked)},r.onclick=()=>{l.checked=!l.checked,si(o.name,l.checked)},i.appendChild(r)}),e.appendChild(i);const s=document.getElementById("visible-presets-count");if(s){const o=n.filter(r=>E.includes(r.name)).length;s.textContent=o}setTimeout(()=>{yt()},50)}function si(e,t){const a=E.indexOf(e);t&&a===-1?E.push(e):!t&&a>-1&&E.splice(a,1),Me(),Yt(),_e();const n=document.getElementById("styles-count");if(n){const{favorites:i,regular:s}=ft(),o=i.length+s.length;n.textContent=o}N&&Ee()}function Yt(){const e=document.getElementById("current-visible-presets-display");if(e){const t=m.filter(n=>!n.internal).length,a=E.length;e.textContent=a===t?"All Visible":`${a} of ${t}`}}function mo(){if(!Ie||!vt)return;const e=document.getElementById("visible-presets-list");!e||e.querySelectorAll(".style-item").length===0||(Y=Math.max(0,Y-1),yt())}function go(){if(!Ie||!vt)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");t.length!==0&&(Y=Math.min(t.length-1,Y+1),yt())}function yt(){if(!Ie)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0)return;t.forEach(n=>{n.classList.remove("menu-selected")}),Y=Math.max(0,Math.min(Y,t.length-1));const a=t[Y];if(a){a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=a.dataset.presetName,i=m.find(o=>o.name===n),s=document.getElementById("visible-presets-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",He===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),He===o?He="":He=o,Y=0,_e()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function po(){if(!Ie||!vt)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0||Y>=t.length)return;const a=t[Y];a&&a.click()}function Yi(){const e=["Very Low","Low","Medium","High","Very High"],t=document.getElementById("current-motion-display");if(t){const a=Math.floor((50-Ze)/10)+1,n=Math.max(1,Math.min(5,a));t.textContent=`Sensitivity: ${e[n-1]}`}}function Mt(){const e={motionThreshold:Ze,motionPixelThreshold:Aa,motionContinuousEnabled:ze,motionCooldown:et,motionStartDelay:tt};try{localStorage.setItem(wi,JSON.stringify(e))}catch(t){console.error("Failed to save motion settings:",t)}}function fo(){try{const e=localStorage.getItem(wi);if(e){const o=JSON.parse(e);Ze=o.motionThreshold||30,Aa=o.motionPixelThreshold||.1,ze=o.motionContinuousEnabled!==void 0?o.motionContinuousEnabled:!0,et=o.motionCooldown||2,tt=o.motionStartDelay||3}const t=document.getElementById("motion-sensitivity-slider");if(t){const o=Math.floor((50-Ze)/10)+1;t.value=Math.max(1,Math.min(5,o))}const a=document.getElementById("motion-continuous-enabled");a&&(a.checked=ze);const n=document.getElementById("motion-cooldown-slider");n&&(n.value=et);const i=document.getElementById("motion-start-delay-slider"),s=document.getElementById("motion-start-delay-value");if(i&&s){const o=ao();i.value=o,s.textContent=mt[o].label}Yi()}catch(e){console.error("Failed to load motion settings:",e)}}function yo(){H=!H;const e=document.getElementById("no-magic-status");e&&(e.textContent=H?"Enabled":"Disabled",e.style.color=H?"#4CAF50":"",e.style.fontWeight=H?"600":"");try{localStorage.setItem(Ii,JSON.stringify(H))}catch(t){console.error("Failed to save No Magic mode:",t)}H?showStatus("No Magic Mode ON - Camera only",2e3):showStatus("No Magic Mode OFF - AI prompts enabled",2e3)}function bo(){try{const e=localStorage.getItem(Ii);if(e!==null){H=JSON.parse(e);const t=document.getElementById("no-magic-status");t&&(t.textContent=H?"Enabled":"Disabled",t.style.color=H?"#4CAF50":"",t.style.fontWeight=H?"600":"")}}catch(e){console.error("Failed to load No Magic mode:",e)}}function To(){document.getElementById("settings-submenu").style.display="none",document.getElementById("tutorial-submenu").style.display="flex",N=!1,qe=!0,ae=0,P=!1,zi()}function Eo(){document.getElementById("tutorial-submenu").style.display="none",qe=!1,ue()}function wo(e){const t=document.getElementById("tutorial-glossary"),a=document.getElementById("tutorial-content-area"),n=document.getElementById("section-"+e),i=document.getElementById("back-to-glossary");t&&a&&n&&(t.style.display="none",a.style.display="flex",i&&(i.style.display="block"),setTimeout(()=>{n.scrollIntoView({behavior:"smooth",block:"start"})},100))}function zi(){const e=document.getElementById("tutorial-glossary"),t=document.getElementById("tutorial-content-area"),a=document.getElementById("back-to-glossary");e&&t&&(t.style.display="none",e.style.display="block",a&&(a.style.display="none"),ae=0,setTimeout(()=>{La()},50))}function Io(){if(!qe)return;const e=document.getElementById("tutorial-glossary");if(e&&e.style.display!=="none"){const n=e.querySelectorAll(".glossary-item");if(n.length===0)return;ae=(ae-1+n.length)%n.length,La();return}const t=document.getElementById("tutorial-content-area");if(!t||t.style.display!=="flex")return;const a=t.querySelector(".submenu-list.tutorial-content");a&&(a.scrollTop=Math.max(0,a.scrollTop-80))}function vo(){if(!qe)return;const e=document.getElementById("tutorial-glossary");if(e&&e.style.display!=="none"){const n=e.querySelectorAll(".glossary-item");if(n.length===0)return;ae=(ae+1)%n.length,La();return}const t=document.getElementById("tutorial-content-area");if(!t||t.style.display!=="flex")return;const a=t.querySelector(".submenu-list.tutorial-content");a&&(a.scrollTop=Math.min(a.scrollHeight-a.clientHeight,a.scrollTop+80))}function La(){const e=document.getElementById("tutorial-glossary");if(!e)return;const t=e.querySelectorAll(".glossary-item");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("menu-selected")}),ae>=0&&ae<t.length)){const a=t[ae];a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"})}}function Ea(){!h||!p||(ge=null,Ct=!1,xt=setInterval(()=>{if(!ce||Ct||!ze&&R.style.display==="block")return;ko()&&(console.log("Motion detected! Capturing..."),zt(),Ct=!0,setTimeout(()=>{if(Ct=!1,ge=null,R.style.display==="block"&&(R.style.display="none",h.style.display="block"),!ze){Ma(),ce=!1;const t=document.getElementById("motion-toggle");t.classList.remove("active"),t.title="Motion Detection: OFF",showStatus("Motion capture complete - Press eye button to reactivate",3e3)}},et*1e3))},500))}function Ma(){xt&&(clearInterval(xt),xt=null),ge=null}function ko(){if(!h||!p)return!1;const e=p.getContext("2d"),t=320,a=240;p.width=t,p.height=a,e.drawImage(h,0,0,t,a);const n=e.getImageData(0,0,t,a);if(!ge)return ge=n,!1;let i=0;const s=t*a;for(let r=0;r<n.data.length;r+=4){const l=Math.abs(n.data[r]-ge.data[r]),c=Math.abs(n.data[r+1]-ge.data[r+1]),u=Math.abs(n.data[r+2]-ge.data[r+2]);(l+c+u)/3>Ze&&i++}return ge=n,i/s>Aa}function Ao(){Je=!Je;const e=document.getElementById("random-toggle");Je?(e.classList.add("random-active"),y.textContent=`Random mode ON -  ${m[T].name}`):(e.classList.remove("random-active"),z()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"random_mode_toggled",enabled:Je,timestamp:Date.now()}))}function Ro(){try{const e=localStorage.getItem(Ai);e&&(I=JSON.parse(e))}catch(e){console.error("Error loading queue:",e),I=[]}}function Ot(){try{localStorage.setItem(Ai,JSON.stringify(I))}catch(e){console.error("Error saving queue:",e)}}function fa(){Be&&(Q?(Be.className="connection-status online",Be.querySelector("#connection-text").textContent="Online"):(Be.className="connection-status offline",Be.querySelector("#connection-text").textContent="Offline"),Be.style.display="block"),st()}function st(){if(Xe){const e=I.length;Xe.querySelector("#queue-count").textContent=e,Xe.style.display=e>0?"block":"none"}if(le){const e=I.length;le.querySelector("#sync-count").textContent=e,le.style.display=e>0&&Q?"block":"none"}}function Oo(){window.addEventListener("online",()=>{Q=!0,fa(),console.log("Connection restored"),I.length>0&&!Fe&&setTimeout(()=>{y.textContent=`Connection restored! Syncing ${I.length} photos...`,Tt()},1e3)}),window.addEventListener("offline",()=>{Q=!1,fa(),console.log("Connection lost"),Fe&&(y.textContent="Connection lost during sync")}),fa()}async function So(){try{return A=(await navigator.mediaDevices.enumerateDevices()).filter(t=>t.kind==="videoinput"),console.log("Available cameras:",A.length),A}catch(e){return console.error("Error enumerating cameras:",e),[]}}function aa(){const e=Et[ye];if(A.length===0)return{video:{facingMode:"environment",width:{ideal:e.width},height:{ideal:e.height}}};const a={video:{deviceId:{exact:A[G].deviceId},width:{ideal:e.width},height:{ideal:e.height}}};return Ye()&&(a.video.advanced=[{zoom:1}]),a}async function No(e){if(!(e===ye||!k)){ye=e,Js(e);try{y.textContent="Changing resolution...",k&&k.getTracks().forEach(a=>a.stop());const t=aa();k=await navigator.mediaDevices.getUserMedia(t),h.srcObject=k,j=k.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(a=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(D),setTimeout(a,100)}catch(n){console.error("Video play error:",n),a()}}}),z(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"resolution_changed",resolution:Et[ye].name,timestamp:Date.now()}))}catch(t){console.error("Resolution change error:",t),y.textContent="Resolution change failed"}}}function wa(){if(A.length===0)return"Default Camera";const e=A[G];let t=e.label;return!t||t===""?e.deviceId?t=`Camera ${G+1}`:t="Unknown Camera":(t=t.replace(/\([^)]*\)/g,"").trim(),t.toLowerCase().includes("front")?t="Front Camera":t.toLowerCase().includes("back")||t.toLowerCase().includes("rear")?t="Back Camera":t.length>20&&(t=t.substring(0,17)+"...")),t}function Ye(){if(A.length===0)return!1;const e=A[G];if(!e)return!1;const t=e.label.toLowerCase();return e.facingMode==="user"?!0:e.facingMode==="environment"?!1:t.includes("front")||t.includes("user")||t.includes("selfie")||t.includes("face")?!0:t.includes("back")||t.includes("rear")||t.includes("environment")?!1:A.length===2?G===1:G>0}function bt(){try{Ye()?(h.style.transform="translateZ(0)",h.style.webkitTransform="translateZ(0)"):(h.style.transform="scaleX(-1) translateZ(0)",h.style.webkitTransform="scaleX(-1) translateZ(0)")}catch(e){console.warn("Mirror transform skipped:",e)}}function Co(){if(!j)return!1;const e=j.getCapabilities();return e&&"zoom"in e}function Vi(){if(!j)return{min:1,max:5,step:.1};const e=j.getCapabilities();return e&&e.zoom?{min:Math.min(e.zoom.min||1,1),max:Math.max(e.zoom.max||5,5),step:e.zoom.step||.1}:{min:1,max:5,step:.1}}async function ot(e){if(j)try{if(Co()){const t=Vi(),a=Math.max(t.min,Math.min(e,t.max)),n={advanced:[{zoom:a}]},i=j.getCapabilities();i&&i.focusMode&&i.focusMode.includes("continuous")&&(n.advanced[0].focusMode="continuous"),await j.applyConstraints(n),D=a,Ye()?(h.style.transform="translateZ(0)",h.style.webkitTransform="translateZ(0)"):(h.style.transform="scaleX(-1) translateZ(0)",h.style.webkitTransform="scaleX(-1) translateZ(0)")}else{const t=Math.max(1,Math.min(e,5));D=t,Ye()?(h.style.transform=`scale(${t})`,h.style.webkitTransform=`scale(${t})`):(h.style.transform=`scaleX(-1) scale(${t})`,h.style.webkitTransform=`scaleX(-1) scale(${t})`)}}catch{const a=Math.max(1,Math.min(e,5));D=a,Ye()?(h.style.transform=`scale(${a})`,h.style.webkitTransform=`scale(${a})`):(h.style.transform=`scaleX(-1) scale(${a})`,h.style.webkitTransform=`scaleX(-1) scale(${a})`)}}async function Ia(){if(j)try{const e=j.getCapabilities();e&&e.focusMode&&(e.focusMode.includes("single-shot")?(await j.applyConstraints({advanced:[{focusMode:"single-shot",zoom:D}]}),console.log("Triggered single-shot focus"),setTimeout(async()=>{try{await j.applyConstraints({advanced:[{focusMode:"continuous",zoom:D}]})}catch(t){console.log("Could not return to continuous focus:",t)}},500)):e.focusMode.includes("manual")&&(await j.applyConstraints({advanced:[{focusMode:"manual",zoom:D}]}),console.log("Triggered manual focus")))}catch(e){console.log("Focus adjustment not supported or failed:",e)}}async function Lo(){if(ua||A.length<=1){console.log("Cannot switch camera: loading or not enough cameras");return}ua=!0;try{y.textContent="Switching camera...",k&&k.getTracks().forEach(t=>t.stop()),G=(G+1)%A.length,console.log(`Switching to camera ${G+1} of ${A.length}`);const e=aa();k=await navigator.mediaDevices.getUserMedia(e),h.srcObject=k,j=k.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(t=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(D),setTimeout(t,100)}catch(a){console.error("Video play error:",a),t()}}}),z(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"camera_switched",cameraIndex:G,cameraLabel:wa(),timestamp:Date.now()}))}catch(e){console.error("Camera switch error:",e),y.textContent="Camera switch failed",G=(G-1+A.length)%A.length}finally{ua=!1}}function Mo(){try{const e=localStorage.getItem(mi);if(e){const t=JSON.parse(e);M=t.count||5;const a=t.speed||2;wt=Qe[a].delay}}catch(e){console.error("Error loading burst settings:",e)}}function oi(e,t){try{localStorage.setItem(mi,JSON.stringify({count:e,speed:t}))}catch(a){console.error("Error saving burst settings:",a)}}function jo(){Re=!Re;const e=document.getElementById("burst-toggle");Re?(e.classList.add("burst-active"),y.textContent=`Burst mode ON (${M} photos) -  ${m[T].name}`):(e.classList.remove("burst-active"),z()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"burst_mode_toggled",enabled:Re,count:M,timestamp:Date.now()}))}function xo(){Oe=!Oe;const e=document.getElementById("timer-toggle");Oe?(e.classList.add("timer-active"),y.textContent=`Timer mode ON (${de}s delay) -  ${m[T].name}`):(e.classList.remove("timer-active"),be&&(clearInterval(be),be=null,document.getElementById("timer-countdown").style.display="none"),z()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"timer_mode_toggled",enabled:Oe,delay:de,timestamp:Date.now()}))}function va(e){let t=de;const a=document.getElementById("timer-countdown"),n=document.getElementById("timer-countdown-text");n.textContent=t,a.style.display="flex",a.classList.remove("countdown-fade-out"),a.classList.add("countdown-fade-in"),y.textContent=`Timer: ${t}s...`,be=setInterval(()=>{t--,t>0?(a.classList.remove("countdown-fade-in"),a.classList.add("countdown-fade-out"),setTimeout(()=>{n.textContent=t,a.classList.remove("countdown-fade-out"),a.classList.add("countdown-fade-in"),y.textContent=`Timer: ${t}s...`},500)):(a.classList.remove("countdown-fade-in"),a.classList.add("countdown-fade-out"),setTimeout(()=>{a.style.display="none",a.classList.remove("countdown-fade-out"),clearInterval(be),be=null,e(),We&&Oe&&(setTimeout(()=>{if(R.style.display==="block"){R.style.display="none",h.style.display="block";const i=document.getElementById("camera-button");i&&A.length>1&&(i.style.display="flex")}},500),setTimeout(()=>{Oe&&va(e)},q*1e3))},500))},1e3)}function Wi(){be&&(clearInterval(be),be=null,document.getElementById("timer-countdown").style.display="none",z())}function Do(){try{const e=localStorage.getItem(gi);if(e){const t=JSON.parse(e);de=t.delay||10,We=t.repeat||!1,q=t.repeatInterval||1}}catch(e){console.error("Error loading timer settings:",e)}}function ya(){try{localStorage.setItem(gi,JSON.stringify({delay:de,repeat:We,repeatInterval:q}))}catch(e){console.error("Error saving timer settings:",e)}}function ut(){const e=document.getElementById("current-timer-display");if(e){const t=We?`Repeat (${yi[Po()].label})`:"No Repeat";e.textContent=`${de}s, ${t}`}}function Po(){for(const[e,t]of Object.entries(yi))if(t.seconds===q)return parseInt(e);return 1}async function ri(){if(!(!k||ma||R.style.display==="block")){ma=!0,y.textContent=`Burst mode: Taking ${M} photos...`;for(let e=0;e<M;e++)y.textContent=`Burst ${e+1}/${M}...`,Uo(e+1),e<M-1&&await new Promise(t=>setTimeout(t,wt));ma=!1,y.textContent=`Burst complete! ${M} photos saved.`,Q&&!Fe?setTimeout(()=>{Tt()},500):Q||(y.textContent=`Burst complete! ${M} photos queued (offline).`),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"burst_complete",count:M,timestamp:Date.now()})),setTimeout(()=>{Re?y.textContent=`Burst mode ON (${M} photos) -  ${m[T].name}`:z()},2e3)}}function Uo(e){if(!k)return;Je&&(T=Gi()),p.width=h.videoWidth,p.height=h.videoHeight;const t=p.getContext("2d",{willReadFrequently:!1,alpha:!1});t.clearRect(0,0,p.width,p.height);const a=p.width/D,n=p.height/D,i=(p.width-a)/2,s=(p.height-n)/2;if(Ye())t.drawImage(h,i,s,a,n,0,0,p.width,p.height);else{t.save(),t.scale(-1,1),t.drawImage(h,i,s,a,n,-p.width,0,p.width,p.height),t.restore();const u=document.createElement("canvas");u.width=p.width,u.height=p.height;const f=u.getContext("2d");f.scale(-1,1),f.drawImage(p,-p.width,0),t.clearRect(0,0,p.width,p.height),t.drawImage(u,0,0)}Pi(t,p.width,p.height);const o=ye>=2?.7:.8,r=p.toDataURL("image/jpeg",o),l=m[T];Li(r);const c={id:Date.now().toString()+"-"+e,imageBase64:r,preset:l,timestamp:Date.now()};I.push(c),Ot(),st()}async function Bo(){try{if(h=document.getElementById("video"),p=document.getElementById("canvas"),R=document.getElementById("captured-image"),y=document.getElementById("status"),Pt=document.getElementById("reset-button"),document.getElementById("start-screen").querySelector(".start-text").textContent="Loading camera...",document.getElementById("start-button").disabled=!0,await So(),A.length>1){const l=A.findIndex(c=>{const u=c.label.toLowerCase();return u.includes("back")||u.includes("rear")||u.includes("environment")});G=l!==-1?l:A.length-1}else G=0;const e=aa();k=await navigator.mediaDevices.getUserMedia(e),h.srcObject=k,j=k.getVideoTracks()[0],setTimeout(()=>{At()},100),console.log("Camera initialized:",wa()),Ro(),Oo(),await new Promise(l=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),ot(1),h.addEventListener("playing",bt),setTimeout(l,100)}catch(c){console.error("Video play error:",c),l()}}}),document.getElementById("start-screen").style.display="none",document.getElementById("camera-container").style.display="flex",y.style.display="block";const t=document.getElementById("camera-button");A.length>1&&(t.style.display="flex");const a=document.getElementById("menu-button");a&&(a.style.display="flex");const n=document.getElementById("burst-toggle");n&&(n.style.display="flex");const i=document.getElementById("random-toggle");i&&(i.style.display="flex");const s=document.getElementById("timer-toggle");s&&(s.style.display="flex");const o=document.getElementById("motion-toggle");o&&(o.style.display="flex");const r=document.getElementById("gallery-button");r&&(r.style.display="flex"),z(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({status:"camera_ready",availableCameras:A.length,currentCamera:wa(),timestamp:Date.now()}))}catch(e){console.error("Camera access error:",e),y.textContent="Camera access denied",typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({status:"camera_error",error:e.message,timestamp:Date.now()}))}}function xe(){k&&h&&(k.getTracks().forEach(e=>{e.stop()}),h.style.display="none",h.srcObject=null)}async function qi(){if(h)try{const e=aa();k=await navigator.mediaDevices.getUserMedia(e),h.srcObject=k,j=k.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(t=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(D),setTimeout(t,100)}catch(a){console.error("Video resume error:",a),t()}}}),h.style.display="block"}catch(e){console.error("Failed to resume camera:",e),y.textContent="Camera resume failed"}}function zt(){if(!k)return;Je&&(T=Gi(),Ra(m[T].name)),p.width=h.videoWidth,p.height=h.videoHeight;const e=p.getContext("2d",{willReadFrequently:!1,alpha:!1});e.clearRect(0,0,p.width,p.height);const t=p.width/D,a=p.height/D,n=(p.width-t)/2,i=(p.height-a)/2;if(Ye())e.drawImage(h,n,i,t,a,0,0,p.width,p.height);else{e.save(),e.scale(-1,1),e.drawImage(h,n,i,t,a,-p.width,0,p.width,p.height),e.restore();const f=document.createElement("canvas");f.width=p.width,f.height=p.height;const v=f.getContext("2d");v.scale(-1,1),v.drawImage(p,-p.width,0),e.clearRect(0,0,p.width,p.height),e.drawImage(f,0,0)}Pi(e,p.width,p.height);const s=ye>=2?.7:.8,o=p.toDataURL("image/jpeg",s);R.src=o,R.style.display="block",R.style.transform="none",h.style.display="none",ce||Oe&&We?Pt.style.display="none":Pt.style.display="block";const r=document.getElementById("camera-button");r&&(r.style.display="none");const l=document.getElementById("resolution-button");l&&(l.style.display="none"),Li(o);const c=m[T],u={id:Date.now().toString(),imageBase64:o,preset:c,timestamp:Date.now()};if(I.push(u),Ot(),st(),Q){const f=H?"Photo saved!":"Photo saved! Uploading...";y.textContent=f,Fe||Tt()}else y.textContent=`Photo queued for sync (${I.length} in queue)`;typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"photo_captured",queued:!0,queueLength:I.length,timestamp:Date.now()}))}async function Tt(){if(I.length===0||Fe)return;if(!Q){y.textContent="Cannot sync - offline";return}Fe=!0,le.disabled=!0,le.classList.add("syncing"),console.log(`Syncing ${I.length} queued photos...`);const e=I.length;let t=0;for(;I.length>0&&Q;){const a=I[0];try{if(y.textContent=`Syncing ${t+1}/${e}...`,typeof PluginMessageHandler<"u"&&!H&&PluginMessageHandler.postMessage(JSON.stringify({message:sa(a.preset.message,a.preset.name),pluginId:"com.r1.pixelart",imageBase64:a.imageBase64})),await new Promise(n=>setTimeout(n,2e3)),Q)I.shift(),t++,Ot(),st();else{console.log("Lost connection during sync");break}await new Promise(n=>setTimeout(n,2e3))}catch(n){console.error("Sync error:",n),y.textContent="Sync error - will retry later";break}}if(Fe=!1,le.disabled=!1,le.classList.remove("syncing"),I.length===0){const a=H?`All ${t} photos saved!`:`All ${t} photos synced successfully!`;y.textContent=a,setTimeout(()=>{z()},2e3)}else Q?y.textContent=`Synced ${t}. ${I.length} remaining.`:y.textContent=`Connection lost. ${I.length} photos queued.`;typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"sync_complete",synced:t,remaining:I.length,timestamp:Date.now()}))}function ja(){const e=document.getElementById("queue-manager"),t=document.getElementById("queue-list");t.innerHTML="",I.length===0?t.innerHTML=`
    ...
    `:I.forEach((a,n)=>{const i=document.createElement("div");i.className="queue-item",i.innerHTML=`
      ...
      `,t.appendChild(i)}),e.style.display="flex"}function Ho(){document.getElementById("queue-manager").style.display="none"}function Go(e){confirm("Remove this photo from the sync queue?")&&(I.splice(e,1),Ot(),st(),ja())}function Fo(e){const t=I[e];alert(`Style: ${t.preset.name}
Prompt: ${t.preset.message}
Saved: ${new Date(t.timestamp).toLocaleString()}`)}function Ki(){confirm("Clear all photos from the queue? This cannot be undone.")&&(I=[],Ot(),st(),ja())}window.addEventListener("sideClick",()=>{if(console.log("Side button pressed"),P){const n=document.getElementById("settings-submenu").querySelectorAll(".menu-section-button");n.length>0&&ee<n.length&&n[ee].click();return}if(Ie){po();return}if(qe){const a=document.getElementById("tutorial-glossary");if(a&&a.style.display!=="none"){const n=a.querySelectorAll(".glossary-item");n.length>0&&ae<n.length&&n[ae].click()}return}if(It){const n=document.getElementById("resolution-submenu").querySelectorAll(".resolution-item");n.length>0&&te<n.length&&n[te].click();return}if(he){Ms();return}if(N&&je){Ws();return}const e=document.getElementById("start-screen"),t=document.getElementById("start-button");if(e&&e.style.display!=="none")console.log("Simulating tap on start button"),setTimeout(()=>{t.click()},100);else if(R&&R.style.display==="block")na();else{if(ce){if(tt>0){let a=tt;const n=document.getElementById("timer-countdown"),i=document.getElementById("timer-countdown-text");i.textContent=a,n.style.display="flex",n.classList.remove("countdown-fade-out"),n.classList.add("countdown-fade-in"),y.textContent=`Motion Detection starting in ${a}s...`,ct=setInterval(()=>{a--,a>0?(n.classList.remove("countdown-fade-in"),n.classList.add("countdown-fade-out"),setTimeout(()=>{i.textContent=a,n.classList.remove("countdown-fade-out"),n.classList.add("countdown-fade-in"),y.textContent=`Motion Detection starting in ${a}s...`},500)):(n.classList.remove("countdown-fade-in"),n.classList.add("countdown-fade-out"),setTimeout(()=>{n.style.display="none",n.classList.remove("countdown-fade-out"),clearInterval(ct),ce&&h&&h.readyState>=2&&(Ea(),showStatus("Motion Detection active - Move in front of camera",3e3))},500))},1e3)}else Ea(),showStatus("Motion Detection ON - Move in front of camera",3e3);return}Oe?va(Re?()=>ri():()=>zt()):Re?ri():zt()}});window.addEventListener("scrollUp",()=>{var t,a,n,i;if(console.log("Scroll wheel: up"),he){ds();return}if(qe){Io();return}if(nt){Mi();return}if(Ie){mo();return}if(N&&je){zs();return}if(Xt){Is();return}if(Jt){Es();return}if(Kt){bs();return}if(qt){fs();return}if(It){gs();return}if(P){us();return}if(((t=document.getElementById("gallery-modal"))==null?void 0:t.style.display)==="flex"){ks();return}if(((a=document.getElementById("image-viewer"))==null?void 0:a.style.display)==="flex"){Rs();return}if(((n=document.getElementById("style-editor"))==null?void 0:n.style.display)==="flex"){Ss();return}if(((i=document.getElementById("queue-manager"))==null?void 0:i.style.display)==="flex"){Cs();return}if(!k||R.style.display==="block")return;const e=Date.now();e-Ht<ki||(Ht=e,Ne&&clearTimeout(Ne),Ne=setTimeout(()=>{let s=Bi();const o=Rt();s=(s-1+o.length)%o.length,T=Hi(s);const r=m[T];r&&Ra(r.name),z(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"preset_changed",preset:m[T].name,timestamp:Date.now()})),Ne=null},50))});window.addEventListener("scrollDown",()=>{var t,a,n,i;if(console.log("Scroll wheel: down"),he){hs();return}if(qe){vo();return}if(nt){ji();return}if(Ie){go();return}if(N&&je){Vs();return}if(Xt){vs();return}if(Jt){ws();return}if(Kt){Ts();return}if(qt){ys();return}if(It){ps();return}if(P){ms();return}if(((t=document.getElementById("gallery-modal"))==null?void 0:t.style.display)==="flex"){As();return}if(((a=document.getElementById("image-viewer"))==null?void 0:a.style.display)==="flex"){Os();return}if(((n=document.getElementById("style-editor"))==null?void 0:n.style.display)==="flex"){Ns();return}if(((i=document.getElementById("queue-manager"))==null?void 0:i.style.display)==="flex"){Ls();return}if(!k||R.style.display==="block")return;const e=Date.now();e-Ht<ki||(Ht=e,Ne&&clearTimeout(Ne),Ne=setTimeout(()=>{let s=Bi();const o=Rt();s=(s+1)%o.length,T=Hi(s);const r=m[T];r&&Ra(r.name),z(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"preset_changed",preset:m[T].name,timestamp:Date.now()})),Ne=null},50))});function z(){T=Math.max(0,Math.min(T,m.length-1));const e=m[T];if(j)try{const t={};j.applyConstraints(t)}catch(t){console.error("Error applying preset constraints:",t)}y&&(y.textContent=`Style: ${e.name}`),localStorage.setItem(pi,T.toString()),N&&it()}window.onPluginMessage=function(e){console.log("Received plugin message:",e),e&&e.status==="processing"?y.textContent="AI is processing your image...":e&&e.status==="complete"?y.textContent="AI transformation complete!":e&&e.error&&(y.textContent="Error: "+e.error)};typeof PluginMessageHandler<"u"?(console.log("Flutter channel is available"),PluginMessageHandler.postMessage(JSON.stringify({message:"AI Camera Styles initialized",pluginId:"com.r1.pixelart"}))):console.log("Running in development mode - Flutter channel not available");function na(){R.style.display="none",ce&&ze||(Ma(),ce&&Ea()),R.style.transform="none",h.style.display="block",Pt.style.display="none";const e=document.getElementById("camera-button");e&&A.length>1&&(e.style.display="flex");const t=document.getElementById("resolution-button");t&&(t.style.display="flex"),setTimeout(()=>{ot(D)},50),z()}function li(e,t){const a=e.clientX-t.clientX,n=e.clientY-t.clientY;return Math.sqrt(a*a+n*n)}function Yo(){const e=document.getElementById("video");e.addEventListener("touchstart",a=>{a.touches.length===2&&(a.preventDefault(),rt=!0,$n=li(a.touches[0],a.touches[1]),Qn=D)},{passive:!1});let t=null;e.addEventListener("touchmove",a=>{if(rt&&a.touches.length===2){a.preventDefault();const i=li(a.touches[0],a.touches[1])/$n,s=Qn*i,o=Vi(),r=Math.max(o.min,Math.min(s,o.max));t||(ot(r),t=setTimeout(()=>{t=null},50))}},{passive:!1}),e.addEventListener("touchend",a=>{a.touches.length<2&&(rt&&Ia(),rt=!1,console.log("Pinch ended, current zoom:",D))}),e.addEventListener("touchcancel",()=>{rt=!1})}function zo(){const e=document.getElementById("video");let t=null,a=!1;e.addEventListener("touchstart",n=>{!N&&R.style.display==="none"&&(a=!1,t=setTimeout(()=>{a=!0;const i=n.touches[0],s=e.getBoundingClientRect(),o=i.clientX-s.left,r=i.clientY-s.top,l=document.createElement("div");l.style.position="absolute",l.style.left=o+"px",l.style.top=r+"px",l.style.width="80px",l.style.height="80px",l.style.border="3px solid #4CAF50",l.style.borderRadius="50%",l.style.transform="translate(-50%, -50%)",l.style.pointerEvents="none",l.style.animation="capturePulse 0.4s ease-out",l.style.zIndex="150",l.style.backgroundColor="rgba(76, 175, 80, 0.2)",document.getElementById("camera-container").appendChild(l),setTimeout(()=>{l.remove()},400),zt(),navigator.vibrate&&navigator.vibrate(50)},500))}),e.addEventListener("touchend",n=>{if(t&&(clearTimeout(t),t=null),!a&&!N&&R.style.display==="none"){Ia();const i=n.changedTouches[0],s=e.getBoundingClientRect(),o=i.clientX-s.left,r=i.clientY-s.top,l=document.createElement("div");l.style.position="absolute",l.style.left=o+"px",l.style.top=r+"px",l.style.width="60px",l.style.height="60px",l.style.border="2px solid #FE5F00",l.style.borderRadius="50%",l.style.transform="translate(-50%, -50%)",l.style.pointerEvents="none",l.style.animation="focusPulse 0.6s ease-out",l.style.zIndex="150",document.getElementById("camera-container").appendChild(l),setTimeout(()=>{l.remove()},600)}}),e.addEventListener("touchcancel",n=>{t&&(clearTimeout(t),t=null)}),e.addEventListener("click",n=>{if(!N&&R.style.display==="none"){Ia();const i=e.getBoundingClientRect(),s=n.clientX-i.left,o=n.clientY-i.top,r=document.createElement("div");r.style.position="absolute",r.style.left=s+"px",r.style.top=o+"px",r.style.width="60px",r.style.height="60px",r.style.border="2px solid #FE5F00",r.style.borderRadius="50%",r.style.transform="translate(-50%, -50%)",r.style.pointerEvents="none",r.style.animation="focusPulse 0.6s ease-out",r.style.zIndex="150",document.getElementById("camera-container").appendChild(r),setTimeout(()=>{r.remove()},600)}})}function ia(){const e=document.getElementById("unified-menu");R&&R.style.display==="block"&&na(),Ee();const t=document.getElementById("styles-count");if(t){const{favorites:a,regular:n}=ft(),i=a.length+n.length;t.textContent=i}Ji(),Vt(),Wt(),ut(),N=!0,je=!0,xe(),Wi(),e.style.display="flex"}async function xa(){N=!1,je=!1,U=0,Ae="",pe="",document.getElementById("style-filter").value="";const e=document.getElementById("menu-category-hint");e&&(e.style.display="none"),document.getElementById("unified-menu").style.display="none",await qi()}function ue(){const e=document.getElementById("settings-submenu"),t=document.getElementById("unified-menu");Ji(),Vt(),ut(),Wt(),t.style.display="none",xe(),e.style.display="flex",N=!1,P=!0,ee=0,setTimeout(()=>{Sa()},50)}function Vo(){document.getElementById("settings-submenu").style.display="none",P=!1,ee=0,ia()}function Wo(){const e=document.getElementById("timer-settings-submenu"),t=document.getElementById("settings-submenu"),a=document.getElementById("timer-delay-slider"),n=document.getElementById("timer-delay-value"),i=document.getElementById("timer-repeat-enabled");if(a&&n){const r=fi.indexOf(de);a.value=r!==-1?r+1:3,n.textContent=de}i&&(i.checked=We);const s=document.getElementById("timer-repeat-interval-input"),o=document.getElementById("timer-repeat-interval-unit");s&&o&&(q>=3600&&q%3600===0?(s.value=q/3600,o.value="3600"):q>=60&&q%60===0?(s.value=q/60,o.value="60"):(s.value=q,o.value="1")),t.style.display="none",xe(),e.style.display="flex",Kt=!0,P=!1}function qo(){document.getElementById("timer-settings-submenu").style.display="none",Kt=!1,ue()}function Ko(){const e=document.querySelector(".styles-menu-scroll-container");e&&(e.scrollTo({top:0,behavior:"smooth"}),U=0,it())}function Jo(){const e=document.querySelector(".styles-menu-scroll-container");if(e){e.scrollTo({top:e.scrollHeight,behavior:"smooth"});const t=document.getElementById("menu-styles-list");if(t){const a=t.querySelectorAll(".style-item");a.length>0&&(U=a.length-1,it())}}}function Ji(){const e=document.getElementById("current-resolution-display");if(e){const t=Et[ye];e.textContent=`${t.width}x${t.height}`}}function Vt(){const e=document.getElementById("current-burst-display");if(e){let t="Medium";for(const[a,n]of Object.entries(Qe))if(n.delay===wt){t=n.label;break}e.textContent=`${M} photos, ${t}`}}function Xo(){document.getElementById("settings-submenu").style.display="none",xe();const e=document.getElementById("resolution-submenu"),t=document.getElementById("resolution-list");t.innerHTML="",Et.forEach((a,n)=>{const i=document.createElement("div");i.className="resolution-item",n===ye&&i.classList.add("active");const s=document.createElement("span");s.className="resolution-name",s.textContent=a.name,i.appendChild(s),i.onclick=()=>{No(n),Xi()},t.appendChild(i)}),e.style.display="flex",It=!0,P=!1,te=0,setTimeout(()=>{const a=e.querySelectorAll(".resolution-item");Na(a)},100)}async function Xi(){document.getElementById("resolution-submenu").style.display="none",It=!1,te=0,ue()}function _o(){document.getElementById("settings-submenu").style.display="none",xe();const e=document.getElementById("burst-submenu"),t=document.getElementById("burst-count-slider"),a=document.getElementById("burst-speed-slider"),n=document.getElementById("burst-count-value"),i=document.getElementById("burst-speed-value");if(t&&n&&(t.value=M,n.textContent=M),a&&i){let s=2;for(const[o,r]of Object.entries(Qe))if(r.delay===wt){s=parseInt(o);break}a.value=s,i.textContent=Qe[s].label}e.style.display="flex",qt=!0,P=!1}async function $o(){document.getElementById("burst-submenu").style.display="none",qt=!1,ue()}function Qo(){document.getElementById("settings-submenu").style.display="none",xe();const e=document.getElementById("master-prompt-submenu"),t=document.getElementById("master-prompt-enabled"),a=document.getElementById("master-prompt-text"),n=document.getElementById("master-prompt-char-count");t&&(t.checked=we),a&&(a.value=Z,a.disabled=!we,n&&(n.textContent=Z.length)),e.style.display="flex",Jt=!0,P=!1}async function Zo(){document.getElementById("master-prompt-submenu").style.display="none",Jt=!1,ue()}function er(){document.getElementById("settings-submenu").style.display="none",xe();const e=document.getElementById("aspect-ratio-submenu");e.style.display="flex",P=!1}async function tr(){document.getElementById("aspect-ratio-submenu").style.display="none",ue()}function ci(){const e=document.getElementById("current-aspect-ratio-display");e&&(e.textContent=F==="none"?"None":F)}function Wt(){const e=document.getElementById("current-master-prompt-display");if(e)if(we&&Z.trim()){const t=Z.substring(0,20);e.textContent=`Enabled: ${t}${Z.length>20?"...":""}`}else we?e.textContent="Enabled (empty)":e.textContent="Disabled"}function jt(){try{localStorage.setItem(bi,Z),localStorage.setItem(Ti,we.toString()),localStorage.setItem(Ei,F)}catch(e){console.error("Failed to save master prompt:",e)}}function ar(){try{const e=localStorage.getItem(bi),t=localStorage.getItem(Ti);e!==null&&(Z=e),t!==null&&(we=t==="true");const a=localStorage.getItem(Ei);if(a){F=a;const n=document.getElementById("aspect-ratio-1-1"),i=document.getElementById("aspect-ratio-16-9");n&&(n.checked=F==="1:1"),i&&(i.checked=F==="16:9");const s=document.getElementById("current-aspect-ratio-display");s&&(s.textContent=F==="none"?"None":F)}}catch(e){console.error("Failed to load master prompt:",e)}}function nr(){try{const e=localStorage.getItem(_i);e&&(ba=JSON.parse(e))}catch(e){console.error("Failed to load selection history:",e),ba={}}}function ir(e){return e?ba[e]||[]:[]}function sa(e,t=""){let a=e;we&&Z.trim()&&(a=`${e} ${Z}`);const n=ir(t),i=Date.now(),s=i%10,o=i%100;return a+=`
