(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function a(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(i){if(i.ep)return;i.ep=!0;const s=a(i);fetch(i.href,s)}})();let h,p,R,y,Bt,I=null,x=null;const wt=[{name:"VGA (640x480)",width:640,height:480},{name:"SVGA (800x600)",width:800,height:600},{name:"XGA (1024x768)",width:1024,height:768},{name:"SXGA (1280x960)",width:1280,height:960},{name:"SXGA+ (1400x1050)",width:1400,height:1050},{name:"UXGA (1600x1200)",width:1600,height:1200},{name:"2K (2048x1080)",width:2048,height:1080},{name:"4K (3840x2160)",width:3840,height:2160}];let ye=0;const di="r1_camera_resolution",Ea=[{name:"Auto",value:"auto"},{name:"Daylight",value:"daylight"},{name:"Cloudy",value:"cloudy"},{name:"Tungsten",value:"tungsten"},{name:"Fluorescent",value:"fluorescent"},{name:"Candlelight",value:"candlelight"},{name:"Moonlight",value:"moonlight"}];let Xe=0;const hi="r1_camera_white_balance";let G=0,A=[],ua=!1,P=1,rt=!1,$n=0,Xn=1,Re=!1,j=5,vt=500,ma=!1;const Qe={1:{delay:800,label:"Slow"},2:{delay:500,label:"Medium"},3:{delay:300,label:"Fast"}},ui="r1_camera_burst_settings",mi="r1_camera_timer_settings",gi="r1_camera_last_preset";let Oe=!1,be=null,de=10,qe=!1,pi=[3,5,10],K=1;const fi={1:{seconds:1,label:"1s"},2:{seconds:3,label:"3s"},3:{seconds:5,label:"5s"},4:{seconds:10,label:"10s"},5:{seconds:30,label:"30s"},6:{seconds:60,label:"1m"},7:{seconds:300,label:"5m"},8:{seconds:600,label:"10m"},9:{seconds:1800,label:"30m"},10:{seconds:3600,label:"1h"}};let Z="",ve=!1;const yi="r1_camera_master_prompt",bi="r1_camera_master_prompt_enabled",Ti="r1_camera_aspect_ratio";let F="none",Je=!1,ce=!1,Lt=null,ge=null,Ze=30,Ia=.1,Ve=!0,et=2,Nt=!1,tt=3;const wi="r1_camera_motion_settings";let ct=null;const mt={1:{seconds:3,label:"3s"},2:{seconds:10,label:"10s"},3:{seconds:30,label:"30s"},4:{seconds:60,label:"1m"},5:{seconds:300,label:"5m"},6:{seconds:600,label:"10m"},7:{seconds:900,label:"15m"},8:{seconds:1800,label:"30m"}};let H=!1;const vi="r1_camera_no_magic_mode";let We,ga,lt=null,U=0,C=!1,xe=!1,he=!1,L=0,ee=0,te=0,B=!1,kt=!1,Kt=!1,Wt=!1,Jt=!1,_t=!1,Ke=!1,nt=!1,re=-1,ae=0;const _i="R1CameraGallery",$i=1,Me="images";let X=null,M=[];const ki="r1_gallery_sort_order";let J=-1,oe=1,Mt=!1,Qn=0,Zn=1,W=1;const Pt=16;let dt=null,ht=null,gt="newest",Ne=!1,S=new Set,$t=!1,Te=[],Ut=null,Ae="",Dt="",ba=0,He="",pe="",Ge="";const ei={transform:"Take a picture and transform the image into [DESCRIBE TRANSFORMATION]. [ADD SPECIFIC DETAILS ABOUT STYLE, APPEARANCE, COLORS, ETC.]",transform_subject:"Take a picture and transform the subject into [WHAT THE SUBJECT BECOMES]. Preserve the subject's recognizable facial structure and identity. [ADD DETAILS ABOUT NEW APPEARANCE, ENVIRONMENT, LIGHTING].",convert:"Take a picture and convert the scene into [DESCRIBE NEW FORMAT/MEDIUM]. [ADD DETAILS ABOUT MATERIALS, TEXTURES, SCALE].",style:"Take a picture in the style of [ARTISTIC STYLE/ARTIST]. [ADD DETAILS ABOUT TECHNIQUE, COLORS, COMPOSITION].",place:"Take a picture and place the subject in [DESCRIBE SCENE/LOCATION]. [ADD DETAILS ABOUT LIGHTING, ATMOSPHERE, INTEGRATION].",recreate:"Take a picture and recreate [FAMOUS WORK/SCENE]. Replace [DESCRIBE WHAT TO REPLACE]. Preserve the iconic [DESCRIBE KEY ELEMENTS TO KEEP].",render:"Take a picture and render it as [FORMAT/MEDIUM]. [ADD DETAILS ABOUT APPEARANCE, TEXTURE, TECHNICAL SPECIFICS].",make:"Take a picture and make the subject into [CHARACTER/CREATURE]. [ADD DETAILS ABOUT APPEARANCE, TRAITS, SETTING]. Make it photorealistic.",analyze:"Analyze the image and [DESCRIBE WHAT TO ANALYZE/EXTRACT]. [ADD DETAILS ABOUT OUTPUT FORMAT] and email it to me.",custom:""},Xi=[{name:"IMPRESSIONISM",category:["ART","PAINTING","CLASSIC"],message:"Take a picture and transform the image into a French Impressionist painting. Preserve subject identity. Use loose, visible brushstrokes, soft edges, and blended colors emphasizing natural light and atmosphere. Painterly oil-on-canvas texture with warm, luminous tones."},{name:"3D PRINT FAILED",category:["HUMOR","TECH","3D"],message:`Take a picture and transform the subject into a real, physical 3D-printed object that has partially failed during printing. The subject must appear made from plastic filament with clearly visible horizontal layer lines across the entire surface.
...
FINAL RESULT:
The image should look like a professional Día de los Muertos portrait — culturally respectful, richly detailed, celebratory, and unmistakably Catrina or Catrín — while preserving the subject’s identity beneath the makeup.`},{name:"DICTIONARY",category:["TEXT","REFERENCE","DEFINITION"],message:"Take a picture and transform it into a dictionary-style entry. Include a small, clearly visible photograph of the subject placed beside or beneath the definition, similar to a modern illustrated dictionary. Allow externally provided language to define the entry name. Use a clean, minimal layout with strong typography. The photo should act as a visual reference for the word, reinforcing the definition."},{name:"DISNEY",category:["ANIMATION","CARTOON","CLASSIC","DISNEY"],message:`Take a picture in the style of a hand-drawn Disney character portrait from the golden age of animation. Smooth linework, expressive poses, elegant shapes, vibrant but flat colors, and classic animation charm.

The subject must be the main character, but include other Disney-style characters in the scene — either original companions or characters inspired by classic Disney films (Cinderella, Snow White, Peter Pan, etc.) — interacting naturally with the subject.

The scene should feel cohesive, storybook-like, and rich in character interaction. Background, props, and lighting should match classic Disney illustration style.
...
The final image should look like a fully integrated Family Guy world with the subject seamlessly placed among other characters, maintaining the show’s humor, proportions, and visual style.`},{name:"FALLOUT",category:["GAME","POSTAPOC","RETRO"],message:"Take a picture and place the subject directly into the Fallout video game world. Transform the subject into either a vault dweller OR a ghoul - choose one based on what would be most interesting. If vault dweller: dress them in iconic blue and yellow Vault-Tec jumpsuit with vault number, equip them with a Pip-Boy device on their wrist showing the green interface screen, give them relatively clean appearance as a vault resident. If ghoul: transform their skin to show radiation damage with peeling, lesions, discoloration, and that characteristic ghoul texture, dress them in tattered wasteland scavenger clothing - ripped shirts, makeshift armor pieces, dirty pre-war clothes. Transform the scene into a post-apocalyptic wasteland environment with destroyed buildings, rusted vehicles, barren landscapes, and nuclear fallout atmosphere. Apply the distinctive Fallout visual style: desaturated greenish-brown color grading, enhanced contrast, subtle film grain, and slight vignette. Include Fallout-specific elements like radiation barrels, Nuka-Cola bottles, retro-futuristic 1950s technology, weathered propaganda posters, wasteland debris, and scattered pre-war artifacts. Maintain photorealistic quality with gritty, dramatic lighting and authentic post-nuclear war atmosphere - do not make it cartoonish."},{name:"FAMILY CREST",category:["HUMOR","HERALDRY","PORTRAIT"],message:`Take a picture of the subject and turn them into the centerpiece of a grand family crest or coat of arms.
...
• Michael Jordan Rookie Era (1984)
• The Steerage (1907)
• Couple in Raccoon Coats (1932)
• Trolley to New Orleans (1955)
• Bricklayer (1928)
• The Hague Street Scene (1930)
...
CRITICAL FACE REPLACEMENT RULE:
• The subject’s face must REPLACE the original subject’s face — not overlay it
• Match exact head angle, expression, focal length, and depth of field
• Skin texture, grain, contrast, and lighting must match the original photo
• No visible seams, masks, glow, blur halos, or mismatched sharpness

HISTORICAL AUTHENTICITY (MANDATORY):
• Preserve the original photograph’s:
  – Camera angle
  – Composition
  – Body pose
  – Clothing
  – Environment
• The subject inherits the original person’s hairstyle, posture, and wardrobe
• Do NOT modernize or stylize anything

PHOTOGRAPHIC PROPERTIES:
• Match the original medium:
  – Black-and-white or color
  – Film grain
  – Motion blur
  – Lens distortion
  – Exposure imperfections
• Aging, scratches, softness, or contrast must match the era

SUBJECT LIKENESS:
• The subject must remain clearly recognizable
• Facial proportions must adapt naturally to the original skull structure
• Avoid caricature, exaggeration, or cartoon qualities

ABSOLUTE PROHIBITIONS:
• NO parody
• NO illustration or painterly style
• NO modern lighting or HDR
• NO exaggerated expressions
• NO changing the scene or adding elements

FINAL RESULT:
The image must look like a genuine archival photograph — as if the subject had truly been present at that historical moment and captured by the original camera.`},{name:"IDIOM",category:["HUMOR","LANGUAGE","LITERAL"],message:"Take a picture and visually interpret a common idiom or phrase in a completely literal way. Place the subject directly inside the literal version of the expression (for example: head in the clouds, walking on thin ice, burning the midnight oil). Preserve realism in lighting and composition while embracing the absurdity of the literal interpretation. The scene should be instantly readable, humorous, and visually clever without requiring any text."},{name:"IF YOU THINK IT'S TRUE, IT'S SNOT",category:["HUMOR","GROSS","EMBARRASSING"],message:`Take a picture and transform it into one of two embarrassing nasal situations - RANDOMLY CHOOSE ONE:
...
Preserve recognizable facial features while adding the unfortunate nasal situation.`},{name:"IKEA INSTRUCTION MANUAL",category:["HUMOR","INSTRUCTIONS","DIAGRAM"],message:"Take a picture and transform the subject into an illustrated instruction manual page, similar to a flat-pack assembly guide. Depict simplified, diagram-style versions of the subject in multiple steps, showing how the subject is assembled or functions. Use clean line art, minimal colors, arrows, step numbers, icons, and humorous warning symbols inspired by the subject’s personality, clothing, or posture. Avoid text-heavy explanations; rely on visual storytelling and pictograms. Present the final image as a complete instruction manual page with a clean layout and playful, clever tone."},{name:"INFINITY MIRROR",category:["ART","OPTICAL","LIGHT","INSTALLATION","SURREAL"],message:`Take a picture and transform the subject into a real infinity-mirror light sculpture.
...
The final image should feel like a dramatic cinematic still, instantly recognizable as the famous scene, but humorous and starring the subject as the protagonist.`},{name:"KISS CAM",category:["HUMOR","SITUATIONAL"],message:`Take a picture of one or more subjects and transform it into a humorous 'Kiss Cam' moment at a sporting event.
...
The final image should look like a candid photograph taken at the exact moment the illusion collapses into disaster — absurd, embarrassing, and realistic.`},{name:"MAGNUM CONTACT SHEET",category:["PHOTOGRAPHY","DOCUMENTARY","JOURNALISM","CONTACT SHEET"],message:`Take a picture and transform it into a Magnum Photos–style contact sheet from a documentary photo shoot.
...
FINAL RESULT:
The image must look like a real Magnum photographer’s contact sheet — raw, observational, imperfect, and revealing the process of capturing a meaningful moment involving the subject.`},{name:"MARGARET KEANE",category:["ART","PAINTING","ARTIST","PORTRAIT","BIG EYES"],message:"Take a picture and transform it into a painting in the distinctive style of Margaret Keane, famous for her 'Big Eyes' artwork. The subject's eyes must be dramatically enlarged - huge, saucer-like eyes taking up a disproportionate amount of the face, approximately 2-3 times normal size with enormous pupils and irises. The eyes should be expressive, soulful, slightly melancholic, and glossy with prominent highlights making them appear wet and luminous. Keep the rest of the facial features - nose, mouth, chin - small and delicate in proportion to the oversized eyes, creating that characteristic waif-like appearance. Use Keane's painting style: smooth oil painting texture, soft blended edges, gentle shading, pastel or muted color palette with emphasis on skin tones. The subject should have a slight tilt to the head, innocent or wistful expression, and that haunting emotional quality. Paint the hair with soft flowing strokes. Background should be simple, atmospheric, often darkly shaded or plain to emphasize the face. Include Keane's signature elements: the enormous plaintive eyes that seem to follow the viewer, childlike innocence mixed with underlying sadness, ethereal dreamy quality, perfect smooth painting technique with no visible brushstrokes. Use lighting that creates soft highlights on the oversized eyes and gentle shadows on the small features. The overall mood should be sweet but slightly melancholic. Make it look like an authentic Margaret Keane painting with that unmistakable 'Big Eyes' style from the 1960s-70s."},{name:"MARIONETTE PUPPET",category:["TOY","PUPPET","STRINGS"],message:"Take a picture and transform the subject into a handcrafted marionette puppet. Preserve recognizable facial features, expressions, proportions, and clothing identity, adapted into carved wood, painted surfaces, stitched fabric, and jointed limbs. Visible strings extend upward from the limbs and head to a wooden marionette controller held above the frame or partially visible. The puppet must show realistic wear, grain, seams, and articulation points. Studio or theatrical lighting with shallow depth of field. The subject must clearly remain the same individual, now reimagined as a controlled puppet."},{name:"MASCOT",category:["SPORTS","CHARACTER","COSTUME"],message:`Take a picture and transform the subject into a realistic, full-body sports team mascot costume. The mascot must appear as a physical, wearable suit with a large oversized mascot head, visible seams, plush or foam textures, and costume proportions typical of real stadium mascots.
...
The sculpture should be clearly recognizable as the subject through its overall form, proportions, and gesture, but rendered as an abstract modernist metalwork piece - an artistic interpretation rather than realistic statue. Make it look like a photograph of an actual large-scale metal sculpture by a contemporary artist, with impressive craftsmanship and artistic vision.`},{name:"MEXICAN MURALISM",category:["ART","PAINTING","MEXICAN","MURAL"],message:"Take a picture and transform the image into Mexican muralism style art. Bold figures, narrative storytelling, strong outlines, social symbolism, and monumental composition."},{name:"MICHELIN MAN",category:["HUMOR","TRANSFORMATION","TIRE","MASCOT"],message:"Take a picture and transform the subject into the Michelin Man (Bibendum) - a figure constructed entirely from stacked white tires. The subject's entire body must be made of puffy white tire segments stacked and arranged to form their shape - arms made of tire rolls, legs made of tire columns, torso built from larger stacked tires, head formed from smaller tire rings. Each body part should clearly show the individual tire construction with visible tread patterns, sidewall ridges, and that characteristic puffy segmented appearance. Preserve the subject's recognizable pose and proportions but render everything as white pneumatic tires. The face should be simple and friendly - tire rings for head with minimal features suggested by tire arrangement, maybe a sash or banner across the chest. The tires should have realistic rubber texture - slightly shiny white rubber surface, tread grooves visible, inflated puffy appearance with slight compression where tires stack. Show dimensional depth with shadows between tire segments. Place the figure in an appropriate setting - tire shop, racing track, automotive environment, or standing proudly as a mascot. Use clean lighting that emphasizes the white tire material and three-dimensional stacked construction. The subject should look jolly and friendly like the actual Michelin Man mascot. Make it look like a photograph of an actual tire sculpture or costume where every part of the body is authentically constructed from real white tires stacked together."},{name:"MIKE JUDGE ME",category:["CARTOON","SATIRE","RETRO","POP CULTURE"],message:`Take a picture and transform the subject into an animated character placed directly inside a scene from the world of Mike Judge cartoons.
...
The final image must look like a real photograph of physical nesting dolls, not illustrated characters.`},{name:"NEWSPAPER",category:["NEWS","VINTAGE","MASTER PROMPT","TEXT","PRINT"],message:"Take a picture and Transform the image into a newspaper print. Halftone dots, grayscale ink. Newsprint texture. Include flavor text headline and story text inspired by the content of the image. The result should be a full view of the newspaper page with the newspaper header and date visible with a relevant background behind the newspaper."},{name:"NIGHTHAWKS",category:["ART","PAINTING","AMERICAN REALISM","HISTORICAL"],message:`Take a picture and transform it into the style of Edward Hopper’s painting *Nighthawks* (1942).

CORE REQUIREMENT:
The subject must REPLACE one of the human figures inside the diner.

SUBJECT INTEGRATION:
• The subject becomes one diner patron or the bartender
• Preserve Hopper’s original body pose, posture, and placement
• Replace ONLY the face with the subject’s likeness, adapted to Hopper’s painted anatomy
...
FINAL RESULT:
The image must look like a faithful Hopper oil painting where the subject appears as if they were always part of *Nighthawks* — quiet, distant, and frozen in time.`},{name:"NIGHT VISION",category:["PHOTOGRAPHY","GREEN","MILITARY"],message:"Take a picture and transform the image into night vision style. Green monochrome, sensor noise."},{name:"NOT IT!",category:["HUMOR","SOCIAL","PHOTO EFFECT"],message:`Take a picture and transform the scene so it appears the subjects are playing the classic game of "Not It."
...
The subject must be the main character, but include other recognizable Pixar-style characters in the scene interacting naturally — they can be from existing Pixar films (Toy Story, Cars, Monsters, Inc., Inside Out, etc.) or original Pixar-style companions.
...
The final image should look like a professional but humorous acting range card, instantly showing how many moods the subject can portray.`},{name:"RAPPER",category:["MUSIC","HIP HOP","URBAN","CONTEMPORARY"],message:`Take a picture and transform the subject into a modern hip-hop rapper with authentic street style and attitude.
...
The final image should be humorous due to contrast with the subject’s age or demeanor, but visually realistic.`},{name:"SCOOBY-DOO ENDING",category:["CARTOON","HUMOR","RETRO"],message:"Take a picture and place the subject in a classic Scooby-Doo unmasking scene ending. Subject shown as the unmasked villain with shocked/guilty expression, surrounded by the Mystery Inc. gang (Fred, Daphne, Velma, Shaggy, and Scooby-Doo) pointing accusingly. Subject drawn in Hanna-Barbera animation style with bold black outlines and flat cel-shaded colors. Include speech bubble with 'And I would have gotten away with it too, if it weren't for you meddling kids!' Classic 1970s Scooby-Doo art style, vintage cartoon aesthetic, iconic reveal moment from the show's ending. Spooky background setting like haunted mansion or abandoned amusement park."},{name:"SCOTT ADAMS' DILBERT",category:["COMIC","CARTOON","OFFICE","SATIRE","CORPORATE"],message:`Take a picture and transform the subject into Scott Adams' distinctive Dilbert comic strip art style.
...
The final image should look like a lifelike puppet replica of the subject, humorous, eerie, and unmistakably artificial.`},{name:"VHS 80s",category:["RETRO","80S","VHS","VIDEO"],message:"Take a picture and transform the image into 1980s VHS footage. Soft focus, scanlines, color bleed, timestamp artifacts. Analog nostalgia aesthetic."},{name:"VICTORIA’S SECRET RUNWAY",category:["HUMOR","FASHION","RUNWAY"],message:`Take a picture of the subject and transform them into a glamorous runway model in the style of a Victoria’s Secret fashion show.
...
The scene should look dramatic, cinematic, and impressive, like a world-class concert starring the subject.`},{name:"VISUAL POETRY",category:["ART","POETRY","METAPHOR"],message:"Take a picture and transform the image into visual poetry. Metaphor-driven imagery with emotional rhythm."},{name:"VITRUVIAN MAN",category:["ART","CLASSICAL","ANATOMY","RENAISSANCE","TRANSFORMATION"],message:`Take a picture and transform the subject into Leonardo da Vinci’s Vitruvian Man.

STYLE & MEDIUM (CRITICAL):
• Hand-drawn Renaissance ink sketch
• Brown or dark sepia ink lines
• Aged parchment or notebook paper background
• Visible paper texture, stains, and imperfections
• No modern illustration, no digital smoothness

ANATOMICAL GEOMETRY:
• The subject must be shown in TWO superimposed poses:
  – One with arms horizontal and legs together (square)
  – One with arms raised and legs apart (circle)
• Both poses must overlap correctly, sharing the same torso
• The subject must be perfectly centered within:
  – A square
  – A circle
• Proportions must follow classical Vitruvian ratios

SUBJECT TRANSFORMATION:
• Replace the Vitruvian Man’s face and body with the subject’s likeness
• Preserve the subject’s facial structure and recognizable features
• The body must be nude in the classical anatomical study sense (no modern censorship clothing)
• Genitalia may be simplified in classical sketch style
• Musculature must be anatomically accurate but sketch-like

DRAWING CONSTRUCTION RULES:
• All forms must be constructed from linework only
• Shading achieved solely via cross-hatching and contour lines
• No color fills, gradients, or painterly shading

TEXT & ANNOTATIONS:
• Include handwritten mirror-script-style anatomical notes around the figure
• Notes should appear observational and scientific
• If master prompt text is provided, reinterpret it as handwritten anatomical commentary
• Otherwise include pseudo-Latin or Italian Renaissance-style annotations

ABSOLUTE PROHIBITIONS:
• NO photographic realism
• NO modern anatomy diagrams
• NO stylized fantasy anatomy
• NO smooth vector art
• NO contemporary typography

FINAL RESULT:
The image must look like a page torn from Leonardo da Vinci’s personal notebook — an anatomical study rendered in Renaissance ink, combining precise geometry, scientific observation, and the subject’s unmistakable likeness.`},{name:"VULCAN TRANSFORMATION",category:["SCI FI","STAR TREK","TRANSFORMATION","ALIEN"],message:"Take a picture and transform the subject into a realistic Vulcan. Preserve the subject’s recognizable facial features, bone structure, body proportions, and personality while adapting them to authentic Vulcan traits including pointed ears, subtle brow structure, calm expression, and controlled posture. Maintain the subject’s clothing identity translated into Vulcan-appropriate garments or Starfleet attire with clean lines and logical design. Neutral, disciplined body language, precise lighting, and understated realism. The subject must appear fully Vulcan, not human with cosmetic alterations."},{name:"WALDO",category:["PUZZLE","HIDDEN","GAME","ILLUSTRATION"],message:`Take a picture and transform it into a classic Where’s Waldo–style illustrated puzzle.
...
Apply realistic cardboard texture, slight wear, subtle creases, and printing imperfections. The box should appear photographed in a real environment. Ensure all text and the subject are readable on a small screen.`},{name:"WHO'S A GOOD HUMAN?",category:["HUMOR","ROLE REVERSAL","SURREAL","PETS"],message:`Take a picture and apply a role-reversal between humans and pets.
...
The scene should look realistic but humorous, with the reactions clearly selling the joke.`},{name:"ZOOM CALL DISASTER",category:["HUMOR","MODERN","RELATABLE","EMBARRASSING"],message:"Take a picture and transform it into a catastrophic Zoom call moment frozen in time. Show the subject in a split-screen grid layout with multiple video call windows. The main window shows the subject caught in an embarrassing situation - maybe in pajama bottoms while standing up, cat walking across keyboard, kid bursting in, unmuted during private moment, or inappropriate background visible. Include other participant windows showing shocked, amused, or secondhand-embarrassment faces reacting. Add authentic Zoom interface elements: mute/video buttons, participant names, 'You are muted' notification, grid view layout, poor lighting, pixelated video quality, frozen buffering frames. Capture that specific modern horror of professional facade collapsing during video conference. Make it look like an actual screenshot of a Zoom disaster moment."},{name:"ZOO-TOPIA",category:["MOVIES","ANIMAL","ANTHROPOMORPHIC","DISNEY"],message:"Take a picture and transform the subject into a realistic anthropomorphized animal character (Zootopia-style realism). Choose an animal form that naturally fits the subject’s facial structure, body type, and personality. Preserve recognizable facial features, expressions, proportions, hairstyle equivalents (fur patterns), and the subject’s exact clothing translated accurately onto the animal anatomy. The character must stand upright with human-like posture, hands, and gestures while retaining believable animal anatomy, fur detail, and species-specific traits. Photorealistic fur, skin, and fabric interaction with cinematic lighting. The subject must clearly remain the same individual, now reimagined as a believable anthropomorphic animal in a realistic world."}];let m=[...Xi],T=0,Se=-1,Q=navigator.onLine,k=[],Fe=!1,Ce=null,Ht=0;const Ei=500,Ii="r1_camera_queue";let De,_e,le;const Ai="r1_camera_styles";let fe=[];const Ri="r1_camera_favorites",Oi="r1_camera_visible_presets";let w=[],ke=!1,z=0,pt="",Et=!0;function Aa(e){lt&&(clearTimeout(lt),lt=null),We||(We=document.getElementById("style-reveal"),ga=document.getElementById("style-reveal-text")),!(!We||!ga)&&(ga.textContent=e,We.style.display="block",lt=setTimeout(()=>{We&&(We.style.display="none"),lt=null},1200))}function Xt(){return new Promise((e,t)=>{const a=indexedDB.open(_i,$i);a.onerror=()=>{console.error("Failed to open IndexedDB:",a.error),t(a.error)},a.onsuccess=()=>{X=a.result,console.log("IndexedDB opened successfully"),e(X)},a.onupgradeneeded=n=>{X=n.target.result,X.objectStoreNames.contains(Me)||(X.createObjectStore(Me,{keyPath:"id"}).createIndex("timestamp","timestamp",{unique:!1}),console.log("Object store created"))}})}async function Qi(){try{const e=localStorage.getItem("r1_gallery_index");if(!e){console.log("No old gallery data to migrate");return}const t=JSON.parse(e);let a=0;for(const n of t){const i="r1_gallery_"+n,s=localStorage.getItem(i);if(s){const o=JSON.parse(s);for(const r of o)await Si(r),a++;localStorage.removeItem(i)}}localStorage.removeItem("r1_gallery_index"),console.log(`Migration complete: ${a} images migrated to IndexedDB`),await Qt()}catch(e){console.error("Migration failed:",e)}}async function Qt(){try{X||await Xt(),M=[];const a=X.transaction([Me],"readonly").objectStore(Me).getAll();return new Promise((n,i)=>{a.onsuccess=()=>{M=a.result||[];const s=localStorage.getItem(ki);s&&(gt=s),M.sort((o,r)=>r.timestamp-o.timestamp),console.log(`Gallery loaded: ${M.length} images`),n()},a.onerror=()=>{console.error("Failed to load gallery:",a.error),M=[],i(a.error)}})}catch(e){console.error("Error loading gallery:",e),M=[]}}async function Si(e){try{X||await Xt();const n=X.transaction([Me],"readwrite").objectStore(Me).add(e);return new Promise((i,s)=>{n.onsuccess=()=>{console.log("Image saved to IndexedDB"),i()},n.onerror=()=>{console.error("Failed to save image:",n.error),s(n.error)}})}catch(t){console.error("Error saving image:",t)}}async function Ci(e){try{X||await Xt();const n=X.transaction([Me],"readwrite").objectStore(Me).delete(e);return new Promise((i,s)=>{n.onsuccess=()=>{console.log("Image deleted from IndexedDB"),i()},n.onerror=()=>{console.error("Failed to delete image:",n.error),s(n.error)}})}catch(t){console.error("Error deleting image:",t)}}async function Ni(e){const t={id:Date.now().toString()+"-"+Math.random().toString(36).substr(2,9),imageBase64:e,timestamp:Date.now()};M.unshift(t),await Si(t),console.log(`Image added. Total: ${M.length}`)}function Zi(e){return!dt&&!ht?e:e.filter(t=>{const a=new Date(t.timestamp);a.setHours(0,0,0,0);const n=a.getTime();let i=!0,s=!0;if(dt){const o=new Date(dt).getTime();i=n>=o}if(ht){const o=new Date(ht).getTime();s=n<=o}return i&&s})}function es(e){const t=[...e];return gt==="newest"?t.sort((a,n)=>n.timestamp-a.timestamp):t.sort((a,n)=>a.timestamp-n.timestamp),t}function Ra(){let e=Zi(M);return es(e)}async function ne(){Le(),Yi(),R&&R.style.display==="block"&&na(),await Qt();const e=document.getElementById("gallery-modal"),t=document.getElementById("gallery-grid"),a=document.getElementById("gallery-pagination"),n=document.getElementById("page-info"),i=document.getElementById("prev-page"),s=document.getElementById("next-page"),o=document.getElementById("gallery-count");o&&(o.textContent=M.length);const r=document.getElementById("gallery-sort-order");r&&(r.value=gt);const l=Ra();if(l.length===0)t.innerHTML='<div class="gallery-empty">No photos match the selected filter.</div>',a.style.display="none";else{const c=Math.ceil(l.length/Pt);W=Math.min(W,c);const u=(W-1)*Pt,f=Math.min(u+Pt,l.length),E=l.slice(u,f),v=document.createDocumentFragment();E.forEach(O=>{const D=document.createElement("div");if(D.className="gallery-item",Ne&&S.has(O.id)&&D.classList.add("selected"),Ne){const Y=document.createElement("input");Y.type="checkbox",Y.className="gallery-item-checkbox",Y.checked=S.has(O.id),Y.addEventListener("click",Ee=>{Ee.stopPropagation(),ai(O.id)}),D.appendChild(Y)}const ie=document.createElement("img");ie.src=O.imageBase64,ie.alt="Gallery image",ie.loading="lazy",D.appendChild(ie),D.onclick=()=>{if(Ne)ai(O.id),ne();else{const Y=M.findIndex(Ee=>Ee.id===O.id);is(Y)}},v.appendChild(D)}),t.innerHTML="",t.appendChild(v),c>1?(a.style.display="flex",n.textContent=`Page ${W} of ${c}`,i.disabled=W===1,s.disabled=W===c):a.style.display="none"}e.style.display="flex"}async function ts(){document.getElementById("gallery-modal").style.display="none",W=1,await qi()}function as(){const e=Ra(),t=Math.ceil(e.length/Pt);W<t&&(W++,ne())}function ns(){W>1&&(W--,ne())}function pa(){W=1,ne()}function ti(e,t){const a=e==="start"?"gallery-start-date-btn":"gallery-end-date-btn",n=document.getElementById(a);if(!n)return;const i=n.querySelector(".date-button-text");if(i)if(t){const o=new Date(t+"T00:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"});i.textContent=o,n.classList.add("has-date")}else i.textContent=e==="start"?"Start":"End",n.classList.remove("has-date")}function is(e){if(e<0||e>=M.length)return;J=e;const t=M[e],a=document.getElementById("image-viewer"),n=document.getElementById("viewer-image"),i=document.getElementById("viewer-prompt");n.src=t.imageBase64,n.style.transform="scale(1) translate(0, 0)",oe=1,i.value="",a.style.display="flex",document.getElementById("gallery-modal").style.display="none"}function ss(){document.getElementById("image-viewer").style.display="none",J=-1,oe=1;const e=document.getElementById("gallery-modal");e.style.display="flex",ne()}async function os(){if(!(J<0||J>=M.length)&&confirm("Delete this image from gallery?")){const e=M[J];await Ci(e.id),M.splice(J,1),document.getElementById("image-viewer").style.display="none",J=-1,oe=1,ne()}}function rs(){const e=document.getElementById("preset-selector");It();const t=document.getElementById("preset-count");t&&(t.textContent=m.length),e.style.display="flex",he=!0,L=0,Ye(),setTimeout(()=>{const a=document.getElementById("preset-list");a&&ba>0&&(a.scrollTop=ba)},50)}function Gt(){const e=document.getElementById("preset-list");e&&(ba=e.scrollTop),document.getElementById("preset-selector").style.display="none",Dt="",Ge="",document.getElementById("preset-filter").value="",he=!1,L=0;const t=document.getElementById("preset-selector-category-hint");t&&(t.style.display="none"),$t=!1}function ls(){if(!he)return;const e=document.getElementById("preset-list");!e||e.querySelectorAll(".preset-item").length===0||(L=Math.max(0,L-1),Ye())}function cs(){if(!he)return;const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");t.length!==0&&(L=Math.min(t.length-1,L+1),Ye())}function Ye(){const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("preset-selected")}),L>=0&&L<t.length)){const a=t[L];a.classList.add("preset-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=a.querySelector(".preset-name").textContent,i=m.find(o=>o.name===n),s=document.getElementById("preset-selector-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",Ge===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),Ge===o?Ge="":Ge=o,L=0,It()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function ds(){if(!B)return;const e=document.getElementById("settings-submenu");!e||e.querySelectorAll(".menu-section-button").length===0||(ee=Math.max(0,ee-1),Oa())}function hs(){if(!B)return;const e=document.getElementById("settings-submenu");if(!e)return;const t=e.querySelectorAll(".menu-section-button");t.length!==0&&(ee=Math.min(t.length-1,ee+1),Oa())}function Oa(){const e=document.getElementById("settings-submenu");if(!e)return;const t=e.querySelectorAll(".menu-section-button");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("menu-selected")}),ee>=0&&ee<t.length)){const a=t[ee];a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"})}}function us(){const e=document.getElementById("resolution-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelectorAll(".resolution-item");t.length!==0&&(te=(te-1+t.length)%t.length,Sa(t))}function ms(){const e=document.getElementById("resolution-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelectorAll(".resolution-item");t.length!==0&&(te=(te+1)%t.length,Sa(t))}function Sa(e){if(e.forEach(t=>t.classList.remove("menu-selected")),te>=0&&te<e.length){const t=e[te];t.classList.add("menu-selected"),t.scrollIntoView({behavior:"smooth",block:"nearest"})}}function gs(){const e=document.getElementById("burst-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ps(){const e=document.getElementById("burst-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function fs(){const e=document.getElementById("timer-settings-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ys(){const e=document.getElementById("timer-settings-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function bs(){const e=document.getElementById("master-prompt-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Ts(){const e=document.getElementById("master-prompt-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function ws(){const e=document.getElementById("motion-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function vs(){const e=document.getElementById("motion-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".submenu-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Mi(){if(!nt)return;const e=document.getElementById("preset-builder-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".preset-builder-form");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function ji(){if(!nt)return;const e=document.getElementById("preset-builder-submenu");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".preset-builder-form");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function ks(){const e=document.getElementById("gallery-modal");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".gallery-scroll-container");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Es(){const e=document.getElementById("gallery-modal");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".gallery-scroll-container");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Is(){const e=document.getElementById("image-viewer");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".viewer-controls");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function As(){const e=document.getElementById("image-viewer");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".viewer-controls");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Rs(){const e=document.getElementById("style-editor");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".style-editor-body");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Os(){const e=document.getElementById("style-editor");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".style-editor-body");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Ss(){const e=document.getElementById("queue-manager");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".queue-list");t&&(t.scrollTop=Math.max(0,t.scrollTop-80))}function Cs(){const e=document.getElementById("queue-manager");if(!e||e.style.display!=="flex")return;const t=e.querySelector(".queue-list");t&&(t.scrollTop=Math.min(t.scrollHeight-t.clientHeight,t.scrollTop+80))}function Ns(){if(!he)return;const e=document.getElementById("preset-list");if(!e)return;const t=e.querySelectorAll(".preset-item");if(t.length===0||L>=t.length)return;const a=t[L];a&&a.click()}function It(){const e=document.getElementById("preset-list");e.innerHTML="";const a=qs().filter(r=>{if(Dt){const l=Dt.toLowerCase(),c=r.category&&r.category.some(f=>f.toLowerCase().includes(l));if(!(r.name.toLowerCase().includes(l)||r.message.toLowerCase().includes(l)||c))return!1}return Ge?r.category&&r.category.includes(Ge):!0}).sort((r,l)=>r.name.localeCompare(l.name)),n=a.filter(r=>at(r.name)),i=a.filter(r=>!at(r.name)),s=[...n,...i];if(s.length===0){e.innerHTML='<div class="preset-empty">No presets found</div>';return}s.forEach(r=>{const l=document.createElement("div");l.className="preset-item";const c=document.createElement("div");c.className="preset-name",c.textContent=r.name;const u=document.createElement("div");u.className="preset-description preset-description-hidden",u.textContent=r.message,l.appendChild(c),l.appendChild(u),l.onclick=()=>{u.classList.contains("preset-description-hidden")?u.classList.remove("preset-description-hidden"):Ms(r)},e.appendChild(l)});const o=document.getElementById("preset-count");o&&(o.textContent=s.length)}async function Ms(e){if($t){const a=Te.findIndex(n=>n.name===e.name);a>-1?Te.splice(a,1):Te.push(e),xi();return}if(window.batchProcessingActive){window.batchProcessingActive=!1;const a=window.batchImagesToProcess;window.batchImagesToProcess=null,Gt();const i=document.getElementById("preset-selector").querySelector(".preset-selector-header h3");i.textContent="Select Preset",await Bs(e,a);return}const t=document.getElementById("viewer-prompt");t.value=e.message,Gt()}function js(){if(J<0||J>=M.length){alert("No image selected");return}const t=document.getElementById("viewer-prompt").value.trim();if(!t){alert("Please enter a prompt or load a preset");return}const a=M[J];typeof PluginMessageHandler<"u"?(PluginMessageHandler.postMessage(JSON.stringify({message:sa(t),pluginId:"com.r1.pixelart",imageBase64:a.imageBase64})),alert("Magic transform submitted! You can submit again with a different prompt.")):alert("Magic transform sent: "+t.substring(0,50)+"...")}function Ft(){Ne=!Ne;const e=document.getElementById("batch-mode-toggle"),t=document.getElementById("batch-controls"),a=document.getElementById("batch-action-bar");Ne?(e.textContent="Done",e.classList.add("active"),t.style.display="flex",a.style.display="flex",S.clear(),Zt(),ne()):(e.textContent="Select",e.classList.remove("active"),t.style.display="none",a.style.display="none",S.clear(),ne())}function Zt(){const e=document.getElementById("batch-selected-count"),t=document.getElementById("batch-apply-preset"),a=document.getElementById("batch-delete");e.textContent=`${S.size} selected`,t.disabled=S.size===0,a&&(a.disabled=S.size===0)}function xs(){const e=Ra();S.clear(),e.forEach(t=>S.add(t.id)),Zt(),ne()}function Ls(){S.clear(),Zt(),ne()}function ai(e){S.has(e)?S.delete(e):S.add(e),Zt()}async function Ps(){if(S.size===0)return;const e=document.getElementById("preset-selector"),t=e.querySelector(".preset-selector-header h3");t.textContent=`Select Preset (${S.size} images)`;const a=Array.from(S);window.batchProcessingActive=!0,window.batchImagesToProcess=a,It(),e.style.display="flex",he=!0,L=0,Ye()}async function Bs(e,t){const a=t||Array.from(S),n=a.length,i=document.createElement("div");i.className="batch-progress-overlay",i.innerHTML=`
  ...
  `,document.body.appendChild(i);let s=0;for(const o of a){const r=M.find(l=>l.id===o);if(r)try{const l=sa(e.message);typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({message:l,pluginId:"com.r1.pixelart",imageBase64:r.imageBase64})),s++,document.getElementById("batch-current").textContent=s,document.getElementById("batch-progress-fill").style.width=`${s/n*100}%`,await new Promise(c=>setTimeout(c,2e3))}catch(l){console.error(`Failed to process image ${o}:`,l)}}document.body.removeChild(i),Ne=!1,S.clear(),Ft(),alert(`Batch processing complete! ${s} of ${n} images submitted.`)}async function Us(){if(S.size===0)return;const e=S.size;if(!confirm(`Are you sure you want to delete ${e} selected image${e>1?"s":""}? This cannot be undone.`))return;const a=Array.from(S),n=document.createElement("div");n.className="batch-progress-overlay",n.innerHTML=`
    <div class="batch-progress-text">Deleting <span id="batch-current">0</span> / ${e}</div>
    <div class="batch-progress-bar">
      <div class="batch-progress-fill" id="batch-progress-fill" style="width: 0%"></div>
    </div>
  `,document.body.appendChild(n);let i=0;for(const s of a)try{await Ci(s),i++,document.getElementById("batch-current").textContent=i,document.getElementById("batch-progress-fill").style.width=`${i/e*100}%`}catch(o){console.error(`Failed to delete image ${s}:`,o)}document.body.removeChild(n),Ne=!1,S.clear(),await Qt(),Ft(),alert(`${i} of ${e} image${i>1?"s":""} deleted successfully.`)}function Ds(e){Ut=e,Te=[],$t=!0;const t=document.getElementById("preset-selector"),a=t.querySelector(".preset-selector-header h3");a.innerHTML='Select Presets (Multi-Select) <span id="multi-preset-count" style="font-size: 12px; color: #666;">(0 selected)</span>';let n=document.getElementById("multi-preset-controls");if(!n){n=document.createElement("div"),n.id="multi-preset-controls",n.style.cssText="padding: 8px; background: #f5f5f5; border-bottom: 1px solid #ddd; display: flex; gap: 8px; justify-content: space-between;",n.innerHTML=`
    ...
    `;const i=document.getElementById("preset-filter");i.parentNode.insertBefore(n,i.nextSibling)}n.style.display="flex",It(),xi(),t.style.display="flex",he=!0,L=0,Ye(),document.getElementById("multi-preset-apply").onclick=Hs,document.getElementById("multi-preset-cancel").onclick=Li}function xi(){document.getElementById("preset-list").querySelectorAll(".preset-item").forEach(n=>{const i=n.querySelector(".preset-name").textContent;Te.some(o=>o.name===i)?(n.style.background="#e8f5e9",n.style.border="2px solid #4CAF50"):(n.style.background="",n.style.border="")});const a=document.getElementById("multi-preset-count");a&&(a.textContent=`(${Te.length} selected)`)}function Li(){$t=!1,Ut=null,Te=[];const e=document.getElementById("multi-preset-controls");e&&(e.style.display="none");const t=document.querySelector(".preset-selector-header h3");t.textContent="Select Preset",Gt()}async function Hs(){if(Te.length===0){alert("Please select at least one preset");return}if(!Ut){alert("No image selected");return}const e=M.find(i=>i.id===Ut);if(!e){alert("Image not found");return}const t=[...Te];Li();const a=document.createElement("div");a.className="batch-progress-overlay",a.innerHTML=`
    <div class="batch-progress-text">Applying preset <span id="batch-current">0</span> / ${t.length}</div>
    <div class="batch-progress-bar">
      <div class="batch-progress-fill" id="batch-progress-fill" style="width: 0%"></div>
    </div>
  `,document.body.appendChild(a);let n=0;for(const i of t)try{const s=sa(i.message);typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({message:s,pluginId:"com.r1.pixelart",imageBase64:e.imageBase64})),n++,document.getElementById("batch-current").textContent=n,document.getElementById("batch-progress-fill").style.width=`${n/t.length*100}%`,await new Promise(o=>setTimeout(o,2e3))}catch(s){console.error(`Failed to apply preset ${i.name}:`,s)}document.body.removeChild(a),alert(`${n} preset${n>1?"s":""} applied successfully!`)}function Gs(){const e=document.getElementById("viewer-image"),t=document.querySelector(".image-viewer-container");let a=0,n=0,i=0,s=0,o=!1;t.addEventListener("touchstart",r=>{r.touches.length===2?(r.preventDefault(),Mt=!0,Qn=ni(r.touches[0],r.touches[1]),Zn=oe):r.touches.length===1&&oe>1&&(o=!0,i=r.touches[0].clientX-a,s=r.touches[0].clientY-n)},{passive:!1}),t.addEventListener("touchmove",r=>{if(Mt&&r.touches.length===2){r.preventDefault();const c=ni(r.touches[0],r.touches[1])/Qn;oe=Math.max(1,Math.min(Zn*c,5)),e.style.transform=`scale(${oe}) translate(${a}px, ${n}px)`}else o&&r.touches.length===1&&oe>1&&(r.preventDefault(),a=r.touches[0].clientX-i,n=r.touches[0].clientY-s,e.style.transform=`scale(${oe}) translate(${a}px, ${n}px)`)},{passive:!1}),t.addEventListener("touchend",r=>{r.touches.length<2&&(Mt=!1),r.touches.length===0&&(o=!1,oe===1&&(a=0,n=0,e.style.transform="scale(1) translate(0, 0)"))}),t.addEventListener("touchcancel",()=>{Mt=!1,o=!1})}function ni(e,t){const a=e.clientX-t.clientX,n=e.clientY-t.clientY;return Math.sqrt(a*a+n*n)}function it(){if(!C)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0)return;t.forEach(n=>{n.classList.remove("menu-selected")}),U=Math.max(0,Math.min(U,t.length-1));const a=t[U];if(a){a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=parseInt(a.dataset.index),i=m[n],s=document.getElementById("menu-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",pe===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),pe===o?pe="":pe=o,U=0,we()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function Fs(){if(!C||!xe)return;const e=document.getElementById("menu-styles-list");!e||e.querySelectorAll(".style-item").length===0||(U=Math.max(0,U-1),it())}function zs(){if(!C||!xe)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");t.length!==0&&(U=Math.min(t.length-1,U+1),it())}function Vs(){if(!C||!xe)return;const e=document.getElementById("menu-styles-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0||U>=t.length)return;const a=t[U];if(a&&a.querySelector(".style-name")){const s=Rt()[U];if(s){const o=m.findIndex(r=>r===s);o!==-1&&(T=o,V(),xa())}}}function Ys(){const e=localStorage.getItem(Ai);if(e)try{const i=JSON.parse(e).filter(s=>s.internal===!1);m.push(...i)}catch(n){console.error("Error loading styles:",n)}const t=localStorage.getItem(Ri);if(t)try{fe=JSON.parse(t),Array.isArray(fe)||(fe=[])}catch(n){console.error("Error parsing favorite styles:",n),fe=[]}Qs(),$s(),Ws();const a=localStorage.getItem(Oi);if(a)try{w=JSON.parse(a),Array.isArray(w)||(w=[])}catch(n){console.error("Error parsing visible presets:",n),w=[]}w.length===0&&(w=m.map(n=>n.name),je())}function je(){try{localStorage.setItem(Oi,JSON.stringify(w))}catch(e){console.error("Error saving visible presets:",e)}}function qs(){return m.filter(e=>w.includes(e.name))}function Ks(e){try{localStorage.setItem(di,e.toString())}catch(t){console.error("Error saving resolution:",t)}}function Ws(){const e=localStorage.getItem(hi);e!==null&&(Xe=parseInt(e))}function Js(){localStorage.setItem(hi,Xe.toString())}function At(){h&&setTimeout(()=>{const e=Ea[Xe];switch(h.style.filter="",e.value){case"daylight":h.style.filter="brightness(1.05) saturate(1.1)";break;case"cloudy":h.style.filter="brightness(1.1) saturate(0.95) sepia(0.05)";break;case"tungsten":h.style.filter="brightness(0.95) saturate(1.15) hue-rotate(-10deg)";break;case"fluorescent":h.style.filter="brightness(1.02) saturate(1.05) hue-rotate(5deg)";break;case"candlelight":h.style.filter="brightness(0.85) saturate(1.3) sepia(0.15) hue-rotate(-15deg)";break;case"moonlight":h.style.filter="brightness(0.7) saturate(0.8) hue-rotate(15deg) contrast(1.1)";break;case"auto":default:h.style.filter="";break}},50)}function Pi(e,t,a){const n=Ea[Xe];if(n.value==="auto")return;const i=e.getImageData(0,0,t,a),s=i.data;let o=1,r=1,l=0,c=1;switch(n.value){case"daylight":o=1.05,r=1.1,l=5;break;case"cloudy":o=1.1,r=.95,l=10;break;case"tungsten":o=.95,r=1.15,l=-20;break;case"fluorescent":o=1.02,r=1.05,l=-10;break;case"candlelight":o=.85,r=1.3,l=25,c=.95;break;case"moonlight":o=.7,r=.8,l=-15,c=1.1;break}for(let u=0;u<s.length;u+=4){let f=s[u],E=s[u+1],v=s[u+2];l>0?(f=Math.min(255,f+l),E=Math.min(255,E+l*.5)):l<0&&(v=Math.min(255,v-l)),f*=o,E*=o,v*=o;const O=.2989*f+.587*E+.114*v;f=O+r*(f-O),E=O+r*(E-O),v=O+r*(v-O),f=((f/255-.5)*c+.5)*255,E=((E/255-.5)*c+.5)*255,v=((v/255-.5)*c+.5)*255,s[u]=Math.max(0,Math.min(255,f)),s[u+1]=Math.max(0,Math.min(255,E)),s[u+2]=Math.max(0,Math.min(255,v))}e.putImageData(i,0,0)}function _s(){document.getElementById("settings-submenu").style.display="none";const e=document.getElementById("white-balance-submenu"),t=document.getElementById("white-balance-list");t.innerHTML="",Ea.forEach((a,n)=>{const i=document.createElement("div");i.className="resolution-item",n===Xe&&i.classList.add("active");const s=document.createElement("span");s.className="resolution-name",s.textContent=a.name,i.appendChild(s),i.onclick=()=>{Xe=n,Js(),document.getElementById("current-white-balance-display").textContent=a.name,I&&At(),Bi()},t.appendChild(i)}),e.style.display="flex"}function Bi(){document.getElementById("white-balance-submenu").style.display="none",document.getElementById("settings-submenu").style.display="flex"}function $s(){try{const e=localStorage.getItem(di);if(e!==null){const t=parseInt(e,10);t>=0&&t<wt.length&&(ye=t)}}catch(e){console.error("Error loading resolution:",e)}}function ft(){const t=m.filter(i=>w.includes(i.name)).slice().sort((i,s)=>i.name.localeCompare(s.name)),a=t.filter(i=>at(i.name)),n=t.filter(i=>!at(i.name));return{favorites:a,regular:n}}function Rt(){const{favorites:e,regular:t}=ft(),a=e.filter(i=>w.includes(i.name)),n=t.filter(i=>w.includes(i.name));return[...a,...n]}function Ui(){const e=Rt(),t=m[T];return e.findIndex(a=>a===t)}function Di(e){const a=Rt()[e];return m.findIndex(n=>n===a)}function ea(){try{const e=m.filter(t=>t.internal===!1);localStorage.setItem(Ai,JSON.stringify(e))}catch(e){console.error("Error saving styles:",e)}}function Xs(e){const t=fe.indexOf(e);t>-1?fe.splice(t,1):fe.push(e),localStorage.setItem(Ri,JSON.stringify(fe));const a=document.querySelector(".styles-menu-scroll-container"),n=a?a.scrollTop:0;we(),a&&(a.scrollTop=n)}function Qs(){const e=localStorage.getItem(gi);if(e!==null)try{const t=parseInt(e,10);t>=0&&t<m.length&&(T=t)}catch(t){console.error("Error loading last used style:",t)}}function at(e){return fe.includes(e)}function Hi(){const e=m.filter(n=>!n.internal&&at(n.name));if(e.length>0){const n=e[Math.floor(Math.random()*e.length)];return m.findIndex(i=>i===n)}const t=m.filter(n=>!n.internal);if(t.length===0)return 0;const a=t[Math.floor(Math.random()*t.length)];return m.findIndex(n=>n===a)}function Zs(){ce=!ce;const e=document.getElementById("motion-toggle");if(ce)e.classList.add("active"),e.title="Motion Detection: ON",showStatus("Motion Detection enabled - Press side button to start",3e3);else{e.classList.remove("active"),e.title="Motion Detection: OFF",Ma(),ct&&(clearInterval(ct),ct=null);const t=document.getElementById("timer-countdown");t&&(t.style.display="none",t.classList.remove("countdown-fade-in","countdown-fade-out"));const a=document.getElementById("camera-button");a&&A.length>1&&(a.style.display="flex"),showStatus("Motion Detection OFF",2e3)}}function eo(){for(let e in mt)if(mt[e].seconds===tt)return parseInt(e);return 1}function to(){document.getElementById("settings-submenu").style.display="none",document.getElementById("motion-submenu").style.display="flex",_t=!0,B=!1}function ao(){document.getElementById("motion-submenu").style.display="none",_t=!1,ue()}function no(){document.getElementById("settings-submenu").style.display="none",document.getElementById("visible-presets-submenu").style.display="flex",C=!1,ke=!0,Et=!0,B=!1,z=0,pt="",document.getElementById("visible-presets-filter").value="",$e(),zt()}function io(){document.getElementById("visible-presets-submenu").style.display="none",ke=!1,Et=!1,z=0,pt="",He="";const e=document.getElementById("visible-presets-category-hint");e&&(e.style.display="none"),ue()}function Gi(){document.getElementById("settings-submenu").style.display="none",document.getElementById("preset-builder-submenu").style.display="flex",C=!1,B=!1,nt=!0,ta()}function Ca(){document.getElementById("preset-builder-submenu").style.display="none",nt=!1,re=-1;const e=document.getElementById("preset-builder-delete");e&&(e.style.display="none"),ue()}function ta(){re=-1,document.getElementById("preset-builder-name").value="",document.getElementById("preset-builder-category").value="",document.getElementById("preset-builder-template").value="",document.getElementById("preset-builder-prompt").value="";const e=document.getElementById("preset-builder-delete");e&&(e.style.display="none");const t=document.getElementById("preset-builder-clear");t&&(t.style.display="flex"),document.querySelectorAll(".chip-section-content").forEach(a=>{a.style.display="none"}),document.querySelectorAll(".chip-section-header").forEach(a=>{a.classList.remove("expanded")})}function so(e){const t=m[e];Gi(),re=e,setTimeout(()=>{const a=document.getElementById("preset-builder-name"),n=document.getElementById("preset-builder-category"),i=document.getElementById("preset-builder-prompt"),s=document.getElementById("preset-builder-template"),o=document.getElementById("preset-builder-delete"),r=document.getElementById("preset-builder-clear");a&&(a.value=t.name),n&&(n.value=t.category?t.category.join(", "):""),i&&(i.value=t.message),s&&(s.value=""),o&&(o.style.display="flex"),r&&(r.style.display="none")},100)}function oo(){const e=document.getElementById("preset-builder-template"),t=document.getElementById("preset-builder-prompt"),a=e.value;a&&ei[a]!==void 0&&(t.value=ei[a])}function ro(){const e=new Set;return m.forEach(t=>{t.category&&Array.isArray(t.category)&&t.category.forEach(a=>{e.add(a.toUpperCase())})}),Array.from(e).sort()}function lo(){const e=document.getElementById("preset-builder-name").value.trim(),t=document.getElementById("preset-builder-category").value.trim(),a=document.getElementById("preset-builder-prompt").value.trim();if(!e){alert("Please enter a preset name");return}if(!a){alert("Please enter a prompt");return}const n=t?t.split(",").map(i=>i.trim().toUpperCase()).filter(i=>i.length>0):["CUSTOM"];if(re>=0){const i=m[re].name;if(m[re]={name:e.toUpperCase(),category:n,message:a,internal:!1},i!==e.toUpperCase()){const s=w.indexOf(i);s>-1&&(w[s]=e.toUpperCase())}}else{const i=m.findIndex(o=>o.name.toUpperCase()===e.toUpperCase());if(i!==-1){if(!confirm(`A preset named "${e}" already exists. Do you want to overwrite it?`))return;m.splice(i,1)}const s={name:e.toUpperCase(),category:n,message:a,internal:!1};m.push(s),w.includes(s.name)||w.push(s.name)}je(),ea(),alert(re>=0?`Preset "${e}" updated!`:`Preset "${e}" saved successfully!`),ta(),Ca(),C&&we()}function co(){if(re<0){alert("No preset selected for deletion");return}const e=m[re];if(e.internal!==!1){alert("Cannot delete built-in presets");return}if(!confirm(`Delete preset "${e.name}"? This cannot be undone.`))return;m.splice(re,1);const t=w.indexOf(e.name);t>-1&&(w.splice(t,1),je()),T>=m.length&&(T=m.length-1),ea(),alert(`Preset "${e.name}" deleted successfully!`),ta(),Ca(),C&&we()}function $e(){const e=document.getElementById("visible-presets-list");e.innerHTML="";const n=m.filter(o=>!o.internal).filter(o=>{if(pt){const r=pt.toLowerCase(),l=o.category&&o.category.some(u=>u.toLowerCase().includes(r));if(!(o.name.toLowerCase().includes(r)||l))return!1}return He?o.category&&o.category.includes(He):!0}).sort((o,r)=>o.name.localeCompare(r.name)),i=document.createDocumentFragment();n.forEach(o=>{const r=document.createElement("div");r.className="style-item",r.dataset.presetName=o.name;const l=document.createElement("input");l.type="checkbox",l.className="master-prompt-checkbox",l.checked=w.includes(o.name),l.style.marginRight="3vw";const c=document.createElement("span");c.className="style-name",c.textContent=o.name,r.appendChild(l),r.appendChild(c),l.onclick=u=>{u.stopPropagation(),ii(o.name,l.checked)},r.onclick=()=>{l.checked=!l.checked,ii(o.name,l.checked)},i.appendChild(r)}),e.appendChild(i);const s=document.getElementById("visible-presets-count");if(s){const o=n.filter(r=>w.includes(r.name)).length;s.textContent=o}setTimeout(()=>{yt()},50)}function ii(e,t){const a=w.indexOf(e);t&&a===-1?w.push(e):!t&&a>-1&&w.splice(a,1),je(),zt(),$e();const n=document.getElementById("styles-count");if(n){const{favorites:i,regular:s}=ft(),o=i.length+s.length;n.textContent=o}C&&we()}function zt(){const e=document.getElementById("current-visible-presets-display");if(e){const t=m.filter(n=>!n.internal).length,a=w.length;e.textContent=a===t?"All Visible":`${a} of ${t}`}}function ho(){if(!ke||!Et)return;const e=document.getElementById("visible-presets-list");!e||e.querySelectorAll(".style-item").length===0||(z=Math.max(0,z-1),yt())}function uo(){if(!ke||!Et)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");t.length!==0&&(z=Math.min(t.length-1,z+1),yt())}function yt(){if(!ke)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0)return;t.forEach(n=>{n.classList.remove("menu-selected")}),z=Math.max(0,Math.min(z,t.length-1));const a=t[z];if(a){a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"});const n=a.dataset.presetName,i=m.find(o=>o.name===n),s=document.getElementById("visible-presets-category-hint");s&&i&&i.category?(s.innerHTML="",s.style.display="block",i.category.forEach((o,r)=>{const l=document.createElement("span");if(l.textContent=o,l.style.cursor="pointer",l.style.padding="0 2px",He===o&&(l.style.textDecoration="underline",l.style.fontWeight="bold"),l.onclick=c=>{c.stopPropagation(),He===o?He="":He=o,z=0,$e()},s.appendChild(l),r<i.category.length-1){const c=document.createElement("span");c.textContent=", ",s.appendChild(c)}})):s&&(s.style.display="none")}}function mo(){if(!ke||!Et)return;const e=document.getElementById("visible-presets-list");if(!e)return;const t=e.querySelectorAll(".style-item");if(t.length===0||z>=t.length)return;const a=t[z];a&&a.click()}function Fi(){const e=["Very Low","Low","Medium","High","Very High"],t=document.getElementById("current-motion-display");if(t){const a=Math.floor((50-Ze)/10)+1,n=Math.max(1,Math.min(5,a));t.textContent=`Sensitivity: ${e[n-1]}`}}function jt(){const e={motionThreshold:Ze,motionPixelThreshold:Ia,motionContinuousEnabled:Ve,motionCooldown:et,motionStartDelay:tt};try{localStorage.setItem(wi,JSON.stringify(e))}catch(t){console.error("Failed to save motion settings:",t)}}function go(){try{const e=localStorage.getItem(wi);if(e){const o=JSON.parse(e);Ze=o.motionThreshold||30,Ia=o.motionPixelThreshold||.1,Ve=o.motionContinuousEnabled!==void 0?o.motionContinuousEnabled:!0,et=o.motionCooldown||2,tt=o.motionStartDelay||3}const t=document.getElementById("motion-sensitivity-slider");if(t){const o=Math.floor((50-Ze)/10)+1;t.value=Math.max(1,Math.min(5,o))}const a=document.getElementById("motion-continuous-enabled");a&&(a.checked=Ve);const n=document.getElementById("motion-cooldown-slider");n&&(n.value=et);const i=document.getElementById("motion-start-delay-slider"),s=document.getElementById("motion-start-delay-value");if(i&&s){const o=eo();i.value=o,s.textContent=mt[o].label}Fi()}catch(e){console.error("Failed to load motion settings:",e)}}function po(){H=!H;const e=document.getElementById("no-magic-status");e&&(e.textContent=H?"Enabled":"Disabled",e.style.color=H?"#4CAF50":"",e.style.fontWeight=H?"600":"");try{localStorage.setItem(vi,JSON.stringify(H))}catch(t){console.error("Failed to save No Magic mode:",t)}H?showStatus("No Magic Mode ON - Camera only",2e3):showStatus("No Magic Mode OFF - AI prompts enabled",2e3)}function fo(){try{const e=localStorage.getItem(vi);if(e!==null){H=JSON.parse(e);const t=document.getElementById("no-magic-status");t&&(t.textContent=H?"Enabled":"Disabled",t.style.color=H?"#4CAF50":"",t.style.fontWeight=H?"600":"")}}catch(e){console.error("Failed to load No Magic mode:",e)}}function yo(){document.getElementById("settings-submenu").style.display="none",document.getElementById("tutorial-submenu").style.display="flex",C=!1,Ke=!0,ae=0,B=!1,zi()}function bo(){document.getElementById("tutorial-submenu").style.display="none",Ke=!1,ue()}function To(e){const t=document.getElementById("tutorial-glossary"),a=document.getElementById("tutorial-content-area"),n=document.getElementById("section-"+e),i=document.getElementById("back-to-glossary");t&&a&&n&&(t.style.display="none",a.style.display="flex",i&&(i.style.display="block"),setTimeout(()=>{n.scrollIntoView({behavior:"smooth",block:"start"})},100))}function zi(){const e=document.getElementById("tutorial-glossary"),t=document.getElementById("tutorial-content-area"),a=document.getElementById("back-to-glossary");e&&t&&(t.style.display="none",e.style.display="block",a&&(a.style.display="none"),ae=0,setTimeout(()=>{Na()},50))}function wo(){if(!Ke)return;const e=document.getElementById("tutorial-glossary");if(e&&e.style.display!=="none"){const n=e.querySelectorAll(".glossary-item");if(n.length===0)return;ae=(ae-1+n.length)%n.length,Na();return}const t=document.getElementById("tutorial-content-area");if(!t||t.style.display!=="flex")return;const a=t.querySelector(".submenu-list.tutorial-content");a&&(a.scrollTop=Math.max(0,a.scrollTop-80))}function vo(){if(!Ke)return;const e=document.getElementById("tutorial-glossary");if(e&&e.style.display!=="none"){const n=e.querySelectorAll(".glossary-item");if(n.length===0)return;ae=(ae+1)%n.length,Na();return}const t=document.getElementById("tutorial-content-area");if(!t||t.style.display!=="flex")return;const a=t.querySelector(".submenu-list.tutorial-content");a&&(a.scrollTop=Math.min(a.scrollHeight-a.clientHeight,a.scrollTop+80))}function Na(){const e=document.getElementById("tutorial-glossary");if(!e)return;const t=e.querySelectorAll(".glossary-item");if(t.length!==0&&(t.forEach(a=>{a.classList.remove("menu-selected")}),ae>=0&&ae<t.length)){const a=t[ae];a.classList.add("menu-selected"),a.scrollIntoView({behavior:"smooth",block:"nearest"})}}function Ta(){!h||!p||(ge=null,Nt=!1,Lt=setInterval(()=>{if(!ce||Nt||!Ve&&R.style.display==="block")return;ko()&&(console.log("Motion detected! Capturing..."),Vt(),Nt=!0,setTimeout(()=>{if(Nt=!1,ge=null,R.style.display==="block"&&(R.style.display="none",h.style.display="block"),!Ve){Ma(),ce=!1;const t=document.getElementById("motion-toggle");t.classList.remove("active"),t.title="Motion Detection: OFF",showStatus("Motion capture complete - Press eye button to reactivate",3e3)}},et*1e3))},500))}function Ma(){Lt&&(clearInterval(Lt),Lt=null),ge=null}function ko(){if(!h||!p)return!1;const e=p.getContext("2d"),t=320,a=240;p.width=t,p.height=a,e.drawImage(h,0,0,t,a);const n=e.getImageData(0,0,t,a);if(!ge)return ge=n,!1;let i=0;const s=t*a;for(let r=0;r<n.data.length;r+=4){const l=Math.abs(n.data[r]-ge.data[r]),c=Math.abs(n.data[r+1]-ge.data[r+1]),u=Math.abs(n.data[r+2]-ge.data[r+2]);(l+c+u)/3>Ze&&i++}return ge=n,i/s>Ia}function Eo(){Je=!Je;const e=document.getElementById("random-toggle");Je?(e.classList.add("random-active"),y.textContent=`Random mode ON • ${m[T].name}`):(e.classList.remove("random-active"),V()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"random_mode_toggled",enabled:Je,timestamp:Date.now()}))}function Io(){try{const e=localStorage.getItem(Ii);e&&(k=JSON.parse(e))}catch(e){console.error("Error loading queue:",e),k=[]}}function Ot(){try{localStorage.setItem(Ii,JSON.stringify(k))}catch(e){console.error("Error saving queue:",e)}}function fa(){De&&(Q?(De.className="connection-status online",De.querySelector("#connection-text").textContent="Online"):(De.className="connection-status offline",De.querySelector("#connection-text").textContent="Offline"),De.style.display="block"),st()}function st(){if(_e){const e=k.length;_e.querySelector("#queue-count").textContent=e,_e.style.display=e>0?"block":"none"}if(le){const e=k.length;le.querySelector("#sync-count").textContent=e,le.style.display=e>0&&Q?"block":"none"}}function Ao(){window.addEventListener("online",()=>{Q=!0,fa(),console.log("Connection restored"),k.length>0&&!Fe&&setTimeout(()=>{y.textContent=`Connection restored! Syncing ${k.length} photos...`,Tt()},1e3)}),window.addEventListener("offline",()=>{Q=!1,fa(),console.log("Connection lost"),Fe&&(y.textContent="Connection lost during sync")}),fa()}async function Ro(){try{return A=(await navigator.mediaDevices.enumerateDevices()).filter(t=>t.kind==="videoinput"),console.log("Available cameras:",A.length),A}catch(e){return console.error("Error enumerating cameras:",e),[]}}function aa(){const e=wt[ye];if(A.length===0)return{video:{facingMode:"environment",width:{ideal:e.width},height:{ideal:e.height}}};const a={video:{deviceId:{exact:A[G].deviceId},width:{ideal:e.width},height:{ideal:e.height}}};return ze()&&(a.video.advanced=[{zoom:1}]),a}async function Oo(e){if(!(e===ye||!I)){ye=e,Ks(e);try{y.textContent="Changing resolution...",I&&I.getTracks().forEach(a=>a.stop());const t=aa();I=await navigator.mediaDevices.getUserMedia(t),h.srcObject=I,x=I.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(a=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(P),setTimeout(a,100)}catch(n){console.error("Video play error:",n),a()}}}),V(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"resolution_changed",resolution:wt[ye].name,timestamp:Date.now()}))}catch(t){console.error("Resolution change error:",t),y.textContent="Resolution change failed"}}}function wa(){if(A.length===0)return"Default Camera";const e=A[G];let t=e.label;return!t||t===""?e.deviceId?t=`Camera ${G+1}`:t="Unknown Camera":(t=t.replace(/\([^)]*\)/g,"").trim(),t.toLowerCase().includes("front")?t="Front Camera":t.toLowerCase().includes("back")||t.toLowerCase().includes("rear")?t="Back Camera":t.length>20&&(t=t.substring(0,17)+"...")),t}function ze(){if(A.length===0)return!1;const e=A[G];if(!e)return!1;const t=e.label.toLowerCase();return e.facingMode==="user"?!0:e.facingMode==="environment"?!1:t.includes("front")||t.includes("user")||t.includes("selfie")||t.includes("face")?!0:t.includes("back")||t.includes("rear")||t.includes("environment")?!1:A.length===2?G===1:G>0}function bt(){try{ze()?(h.style.transform="translateZ(0)",h.style.webkitTransform="translateZ(0)"):(h.style.transform="scaleX(-1) translateZ(0)",h.style.webkitTransform="scaleX(-1) translateZ(0)")}catch(e){console.warn("Mirror transform skipped:",e)}}function So(){if(!x)return!1;const e=x.getCapabilities();return e&&"zoom"in e}function Vi(){if(!x)return{min:1,max:5,step:.1};const e=x.getCapabilities();return e&&e.zoom?{min:Math.min(e.zoom.min||1,1),max:Math.max(e.zoom.max||5,5),step:e.zoom.step||.1}:{min:1,max:5,step:.1}}async function ot(e){if(x)try{if(So()){const t=Vi(),a=Math.max(t.min,Math.min(e,t.max)),n={advanced:[{zoom:a}]},i=x.getCapabilities();i&&i.focusMode&&i.focusMode.includes("continuous")&&(n.advanced[0].focusMode="continuous"),await x.applyConstraints(n),P=a,ze()?(h.style.transform="translateZ(0)",h.style.webkitTransform="translateZ(0)"):(h.style.transform="scaleX(-1) translateZ(0)",h.style.webkitTransform="scaleX(-1) translateZ(0)")}else{const t=Math.max(1,Math.min(e,5));P=t,ze()?(h.style.transform=`scale(${t})`,h.style.webkitTransform=`scale(${t})`):(h.style.transform=`scaleX(-1) scale(${t})`,h.style.webkitTransform=`scaleX(-1) scale(${t})`)}}catch{const a=Math.max(1,Math.min(e,5));P=a,ze()?(h.style.transform=`scale(${a})`,h.style.webkitTransform=`scale(${a})`):(h.style.transform=`scaleX(-1) scale(${a})`,h.style.webkitTransform=`scaleX(-1) scale(${a})`)}}async function va(){if(x)try{const e=x.getCapabilities();e&&e.focusMode&&(e.focusMode.includes("single-shot")?(await x.applyConstraints({advanced:[{focusMode:"single-shot",zoom:P}]}),console.log("Triggered single-shot focus"),setTimeout(async()=>{try{await x.applyConstraints({advanced:[{focusMode:"continuous",zoom:P}]})}catch(t){console.log("Could not return to continuous focus:",t)}},500)):e.focusMode.includes("manual")&&(await x.applyConstraints({advanced:[{focusMode:"manual",zoom:P}]}),console.log("Triggered manual focus")))}catch(e){console.log("Focus adjustment not supported or failed:",e)}}async function Co(){if(ua||A.length<=1){console.log("Cannot switch camera: loading or not enough cameras");return}ua=!0;try{y.textContent="Switching camera...",I&&I.getTracks().forEach(t=>t.stop()),G=(G+1)%A.length,console.log(`Switching to camera ${G+1} of ${A.length}`);const e=aa();I=await navigator.mediaDevices.getUserMedia(e),h.srcObject=I,x=I.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(t=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(P),setTimeout(t,100)}catch(a){console.error("Video play error:",a),t()}}}),V(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"camera_switched",cameraIndex:G,cameraLabel:wa(),timestamp:Date.now()}))}catch(e){console.error("Camera switch error:",e),y.textContent="Camera switch failed",G=(G-1+A.length)%A.length}finally{ua=!1}}function No(){try{const e=localStorage.getItem(ui);if(e){const t=JSON.parse(e);j=t.count||5;const a=t.speed||2;vt=Qe[a].delay}}catch(e){console.error("Error loading burst settings:",e)}}function si(e,t){try{localStorage.setItem(ui,JSON.stringify({count:e,speed:t}))}catch(a){console.error("Error saving burst settings:",a)}}function Mo(){Re=!Re;const e=document.getElementById("burst-toggle");Re?(e.classList.add("burst-active"),y.textContent=`Burst mode ON (${j} photos) • ${m[T].name}`):(e.classList.remove("burst-active"),V()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"burst_mode_toggled",enabled:Re,count:j,timestamp:Date.now()}))}function jo(){Oe=!Oe;const e=document.getElementById("timer-toggle");Oe?(e.classList.add("timer-active"),y.textContent=`Timer mode ON (${de}s delay) • ${m[T].name}`):(e.classList.remove("timer-active"),be&&(clearInterval(be),be=null,document.getElementById("timer-countdown").style.display="none"),V()),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"timer_mode_toggled",enabled:Oe,delay:de,timestamp:Date.now()}))}function ka(e){let t=de;const a=document.getElementById("timer-countdown"),n=document.getElementById("timer-countdown-text");n.textContent=t,a.style.display="flex",a.classList.remove("countdown-fade-out"),a.classList.add("countdown-fade-in"),y.textContent=`Timer: ${t}s...`,be=setInterval(()=>{t--,t>0?(a.classList.remove("countdown-fade-in"),a.classList.add("countdown-fade-out"),setTimeout(()=>{n.textContent=t,a.classList.remove("countdown-fade-out"),a.classList.add("countdown-fade-in"),y.textContent=`Timer: ${t}s...`},500)):(a.classList.remove("countdown-fade-in"),a.classList.add("countdown-fade-out"),setTimeout(()=>{a.style.display="none",a.classList.remove("countdown-fade-out"),clearInterval(be),be=null,e(),qe&&Oe&&(setTimeout(()=>{if(R.style.display==="block"){R.style.display="none",h.style.display="block";const i=document.getElementById("camera-button");i&&A.length>1&&(i.style.display="flex")}},500),setTimeout(()=>{Oe&&ka(e)},K*1e3))},500))},1e3)}function Yi(){be&&(clearInterval(be),be=null,document.getElementById("timer-countdown").style.display="none",V())}function xo(){try{const e=localStorage.getItem(mi);if(e){const t=JSON.parse(e);de=t.delay||10,qe=t.repeat||!1,K=t.repeatInterval||1}}catch(e){console.error("Error loading timer settings:",e)}}function ya(){try{localStorage.setItem(mi,JSON.stringify({delay:de,repeat:qe,repeatInterval:K}))}catch(e){console.error("Error saving timer settings:",e)}}function ut(){const e=document.getElementById("current-timer-display");if(e){const t=qe?`Repeat (${fi[Lo()].label})`:"No Repeat";e.textContent=`${de}s, ${t}`}}function Lo(){for(const[e,t]of Object.entries(fi))if(t.seconds===K)return parseInt(e);return 1}async function oi(){if(!(!I||ma||R.style.display==="block")){ma=!0,y.textContent=`Burst mode: Taking ${j} photos...`;for(let e=0;e<j;e++)y.textContent=`Burst ${e+1}/${j}...`,Po(e+1),e<j-1&&await new Promise(t=>setTimeout(t,vt));ma=!1,y.textContent=`Burst complete! ${j} photos saved.`,Q&&!Fe?setTimeout(()=>{Tt()},500):Q||(y.textContent=`Burst complete! ${j} photos queued (offline).`),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"burst_complete",count:j,timestamp:Date.now()})),setTimeout(()=>{Re?y.textContent=`Burst mode ON (${j} photos) • ${m[T].name}`:V()},2e3)}}function Po(e){if(!I)return;Je&&(T=Hi()),p.width=h.videoWidth,p.height=h.videoHeight;const t=p.getContext("2d",{willReadFrequently:!1,alpha:!1});t.clearRect(0,0,p.width,p.height);const a=p.width/P,n=p.height/P,i=(p.width-a)/2,s=(p.height-n)/2;if(ze())t.drawImage(h,i,s,a,n,0,0,p.width,p.height);else{t.save(),t.scale(-1,1),t.drawImage(h,i,s,a,n,-p.width,0,p.width,p.height),t.restore();const u=document.createElement("canvas");u.width=p.width,u.height=p.height;const f=u.getContext("2d");f.scale(-1,1),f.drawImage(p,-p.width,0),t.clearRect(0,0,p.width,p.height),t.drawImage(u,0,0)}Pi(t,p.width,p.height);const o=ye>=2?.7:.8,r=p.toDataURL("image/jpeg",o),l=m[T];Ni(r);const c={id:Date.now().toString()+"-"+e,imageBase64:r,preset:l,timestamp:Date.now()};k.push(c),Ot(),st()}async function Bo(){try{if(h=document.getElementById("video"),p=document.getElementById("canvas"),R=document.getElementById("captured-image"),y=document.getElementById("status"),Bt=document.getElementById("reset-button"),document.getElementById("start-screen").querySelector(".start-text").textContent="Loading camera...",document.getElementById("start-button").disabled=!0,await Ro(),A.length>1){const l=A.findIndex(c=>{const u=c.label.toLowerCase();return u.includes("back")||u.includes("rear")||u.includes("environment")});G=l!==-1?l:A.length-1}else G=0;const e=aa();I=await navigator.mediaDevices.getUserMedia(e),h.srcObject=I,x=I.getVideoTracks()[0],setTimeout(()=>{At()},100),console.log("Camera initialized:",wa()),Io(),Ao(),await new Promise(l=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),ot(1),h.addEventListener("playing",bt),setTimeout(l,100)}catch(c){console.error("Video play error:",c),l()}}}),document.getElementById("start-screen").style.display="none",document.getElementById("camera-container").style.display="flex",y.style.display="block";const t=document.getElementById("camera-button");A.length>1&&(t.style.display="flex");const a=document.getElementById("menu-button");a&&(a.style.display="flex");const n=document.getElementById("burst-toggle");n&&(n.style.display="flex");const i=document.getElementById("random-toggle");i&&(i.style.display="flex");const s=document.getElementById("timer-toggle");s&&(s.style.display="flex");const o=document.getElementById("motion-toggle");o&&(o.style.display="flex");const r=document.getElementById("gallery-button");r&&(r.style.display="flex"),V(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({status:"camera_ready",availableCameras:A.length,currentCamera:wa(),timestamp:Date.now()}))}catch(e){console.error("Camera access error:",e),y.textContent="Camera access denied",typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({status:"camera_error",error:e.message,timestamp:Date.now()}))}}function Le(){I&&h&&(I.getTracks().forEach(e=>{e.stop()}),h.style.display="none",h.srcObject=null)}async function qi(){if(h)try{const e=aa();I=await navigator.mediaDevices.getUserMedia(e),h.srcObject=I,x=I.getVideoTracks()[0],setTimeout(()=>{At()},100),await new Promise(t=>{h.onloadedmetadata=async()=>{try{await h.play(),bt(),await ot(P),setTimeout(t,100)}catch(a){console.error("Video resume error:",a),t()}}}),h.style.display="block"}catch(e){console.error("Failed to resume camera:",e),y.textContent="Camera resume failed"}}function Vt(){if(!I)return;Je&&(T=Hi(),Aa(m[T].name)),p.width=h.videoWidth,p.height=h.videoHeight;const e=p.getContext("2d",{willReadFrequently:!1,alpha:!1});e.clearRect(0,0,p.width,p.height);const t=p.width/P,a=p.height/P,n=(p.width-t)/2,i=(p.height-a)/2;if(ze())e.drawImage(h,n,i,t,a,0,0,p.width,p.height);else{e.save(),e.scale(-1,1),e.drawImage(h,n,i,t,a,-p.width,0,p.width,p.height),e.restore();const f=document.createElement("canvas");f.width=p.width,f.height=p.height;const E=f.getContext("2d");E.scale(-1,1),E.drawImage(p,-p.width,0),e.clearRect(0,0,p.width,p.height),e.drawImage(f,0,0)}Pi(e,p.width,p.height);const s=ye>=2?.7:.8,o=p.toDataURL("image/jpeg",s);R.src=o,R.style.display="block",R.style.transform="none",h.style.display="none",ce||Oe&&qe?Bt.style.display="none":Bt.style.display="block";const r=document.getElementById("camera-button");r&&(r.style.display="none");const l=document.getElementById("resolution-button");l&&(l.style.display="none"),Ni(o);const c=m[T],u={id:Date.now().toString(),imageBase64:o,preset:c,timestamp:Date.now()};if(k.push(u),Ot(),st(),Q){const f=H?"Photo saved!":"Photo saved! Uploading...";y.textContent=f,Fe||Tt()}else y.textContent=`Photo queued for sync (${k.length} in queue)`;typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"photo_captured",queued:!0,queueLength:k.length,timestamp:Date.now()}))}async function Tt(){if(k.length===0||Fe)return;if(!Q){y.textContent="Cannot sync - offline";return}Fe=!0,le.disabled=!0,le.classList.add("syncing"),console.log(`Syncing ${k.length} queued photos...`);const e=k.length;let t=0;for(;k.length>0&&Q;){const a=k[0];try{if(y.textContent=`Syncing ${t+1}/${e}...`,typeof PluginMessageHandler<"u"&&!H&&PluginMessageHandler.postMessage(JSON.stringify({message:sa(a.preset.message),pluginId:"com.r1.pixelart",imageBase64:a.imageBase64})),await new Promise(n=>setTimeout(n,2e3)),Q)k.shift(),t++,Ot(),st();else{console.log("Lost connection during sync");break}await new Promise(n=>setTimeout(n,2e3))}catch(n){console.error("Sync error:",n),y.textContent="Sync error - will retry later";break}}if(Fe=!1,le.disabled=!1,le.classList.remove("syncing"),k.length===0){const a=H?`All ${t} photos saved!`:`All ${t} photos synced successfully!`;y.textContent=a,setTimeout(()=>{V()},2e3)}else Q?y.textContent=`Synced ${t}. ${k.length} remaining.`:y.textContent=`Connection lost. ${k.length} photos queued.`;typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"sync_complete",synced:t,remaining:k.length,timestamp:Date.now()}))}function ja(){const e=document.getElementById("queue-manager"),t=document.getElementById("queue-list");t.innerHTML="",k.length===0?t.innerHTML=`
      <div class="queue-empty">
        <h4>No Photos in Queue</h4>
        <p>Take photos while offline and they'll appear here for syncing.</p>
      </div>
    `:k.forEach((a,n)=>{const i=document.createElement("div");i.className="queue-item",i.innerHTML=`
      ...
      `,t.appendChild(i)}),e.style.display="flex"}function Uo(){document.getElementById("queue-manager").style.display="none"}function Do(e){confirm("Remove this photo from the sync queue?")&&(k.splice(e,1),Ot(),st(),ja())}function Ho(e){const t=k[e];alert(`Style: ${t.preset.name}
Prompt: ${t.preset.message}
Saved: ${new Date(t.timestamp).toLocaleString()}`)}function Ki(){confirm("Clear all photos from the queue? This cannot be undone.")&&(k=[],Ot(),st(),ja())}window.addEventListener("sideClick",()=>{if(console.log("Side button pressed"),B){const n=document.getElementById("settings-submenu").querySelectorAll(".menu-section-button");n.length>0&&ee<n.length&&n[ee].click();return}if(ke){mo();return}if(Ke){const a=document.getElementById("tutorial-glossary");if(a&&a.style.display!=="none"){const n=a.querySelectorAll(".glossary-item");n.length>0&&ae<n.length&&n[ae].click()}return}if(kt){const n=document.getElementById("resolution-submenu").querySelectorAll(".resolution-item");n.length>0&&te<n.length&&n[te].click();return}if(he){Ns();return}if(C&&xe){Vs();return}const e=document.getElementById("start-screen"),t=document.getElementById("start-button");if(e&&e.style.display!=="none")console.log("Simulating tap on start button"),setTimeout(()=>{t.click()},100);else if(R&&R.style.display==="block")na();else{if(ce){if(tt>0){let a=tt;const n=document.getElementById("timer-countdown"),i=document.getElementById("timer-countdown-text");i.textContent=a,n.style.display="flex",n.classList.remove("countdown-fade-out"),n.classList.add("countdown-fade-in"),y.textContent=`Motion Detection starting in ${a}s...`,ct=setInterval(()=>{a--,a>0?(n.classList.remove("countdown-fade-in"),n.classList.add("countdown-fade-out"),setTimeout(()=>{i.textContent=a,n.classList.remove("countdown-fade-out"),n.classList.add("countdown-fade-in"),y.textContent=`Motion Detection starting in ${a}s...`},500)):(n.classList.remove("countdown-fade-in"),n.classList.add("countdown-fade-out"),setTimeout(()=>{n.style.display="none",n.classList.remove("countdown-fade-out"),clearInterval(ct),ce&&h&&h.readyState>=2&&(Ta(),showStatus("Motion Detection active - Move in front of camera",3e3))},500))},1e3)}else Ta(),showStatus("Motion Detection ON - Move in front of camera",3e3);return}Oe?ka(Re?()=>oi():()=>Vt()):Re?oi():Vt()}});window.addEventListener("scrollUp",()=>{var t,a,n,i;if(console.log("Scroll wheel: up"),he){ls();return}if(Ke){wo();return}if(nt){Mi();return}if(ke){ho();return}if(C&&xe){Fs();return}if(_t){ws();return}if(Jt){bs();return}if(Wt){fs();return}if(Kt){gs();return}if(kt){us();return}if(B){ds();return}if(((t=document.getElementById("gallery-modal"))==null?void 0:t.style.display)==="flex"){ks();return}if(((a=document.getElementById("image-viewer"))==null?void 0:a.style.display)==="flex"){Is();return}if(((n=document.getElementById("style-editor"))==null?void 0:n.style.display)==="flex"){Rs();return}if(((i=document.getElementById("queue-manager"))==null?void 0:i.style.display)==="flex"){Ss();return}if(!I||R.style.display==="block")return;const e=Date.now();e-Ht<Ei||(Ht=e,Ce&&clearTimeout(Ce),Ce=setTimeout(()=>{let s=Ui();const o=Rt();s=(s-1+o.length)%o.length,T=Di(s);const r=m[T];r&&Aa(r.name),V(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"preset_changed",preset:m[T].name,timestamp:Date.now()})),Ce=null},50))});window.addEventListener("scrollDown",()=>{var t,a,n,i;if(console.log("Scroll wheel: down"),he){cs();return}if(Ke){vo();return}if(nt){ji();return}if(ke){uo();return}if(C&&xe){zs();return}if(_t){vs();return}if(Jt){Ts();return}if(Wt){ys();return}if(Kt){ps();return}if(kt){ms();return}if(B){hs();return}if(((t=document.getElementById("gallery-modal"))==null?void 0:t.style.display)==="flex"){Es();return}if(((a=document.getElementById("image-viewer"))==null?void 0:a.style.display)==="flex"){As();return}if(((n=document.getElementById("style-editor"))==null?void 0:n.style.display)==="flex"){Os();return}if(((i=document.getElementById("queue-manager"))==null?void 0:i.style.display)==="flex"){Cs();return}if(!I||R.style.display==="block")return;const e=Date.now();e-Ht<Ei||(Ht=e,Ce&&clearTimeout(Ce),Ce=setTimeout(()=>{let s=Ui();const o=Rt();s=(s+1)%o.length,T=Di(s);const r=m[T];r&&Aa(r.name),V(),typeof PluginMessageHandler<"u"&&PluginMessageHandler.postMessage(JSON.stringify({action:"preset_changed",preset:m[T].name,timestamp:Date.now()})),Ce=null},50))});function V(){T=Math.max(0,Math.min(T,m.length-1));const e=m[T];if(x)try{const t={};x.applyConstraints(t)}catch(t){console.error("Error applying preset constraints:",t)}y&&(y.textContent=`Style: ${e.name}`),localStorage.setItem(gi,T.toString()),C&&it()}window.onPluginMessage=function(e){console.log("Received plugin message:",e),e&&e.status==="processing"?y.textContent="AI is processing your image...":e&&e.status==="complete"?y.textContent="AI transformation complete!":e&&e.error&&(y.textContent="Error: "+e.error)};typeof PluginMessageHandler<"u"?(console.log("Flutter channel is available"),PluginMessageHandler.postMessage(JSON.stringify({message:"AI Camera Styles initialized",pluginId:"com.r1.pixelart"}))):console.log("Running in development mode - Flutter channel not available");function na(){R.style.display="none",ce&&Ve||(Ma(),ce&&Ta()),R.style.transform="none",h.style.display="block",Bt.style.display="none";const e=document.getElementById("camera-button");e&&A.length>1&&(e.style.display="flex");const t=document.getElementById("resolution-button");t&&(t.style.display="flex"),setTimeout(()=>{ot(P)},50),V()}function ri(e,t){const a=e.clientX-t.clientX,n=e.clientY-t.clientY;return Math.sqrt(a*a+n*n)}function Go(){const e=document.getElementById("video");e.addEventListener("touchstart",a=>{a.touches.length===2&&(a.preventDefault(),rt=!0,$n=ri(a.touches[0],a.touches[1]),Xn=P)},{passive:!1});let t=null;e.addEventListener("touchmove",a=>{if(rt&&a.touches.length===2){a.preventDefault();const i=ri(a.touches[0],a.touches[1])/$n,s=Xn*i,o=Vi(),r=Math.max(o.min,Math.min(s,o.max));t||(ot(r),t=setTimeout(()=>{t=null},50))}},{passive:!1}),e.addEventListener("touchend",a=>{a.touches.length<2&&(rt&&va(),rt=!1,console.log("Pinch ended, current zoom:",P))}),e.addEventListener("touchcancel",()=>{rt=!1})}function Fo(){const e=document.getElementById("video");let t=null,a=!1;e.addEventListener("touchstart",n=>{!C&&R.style.display==="none"&&(a=!1,t=setTimeout(()=>{a=!0;const i=n.touches[0],s=e.getBoundingClientRect(),o=i.clientX-s.left,r=i.clientY-s.top,l=document.createElement("div");l.style.position="absolute",l.style.left=o+"px",l.style.top=r+"px",l.style.width="80px",l.style.height="80px",l.style.border="3px solid #4CAF50",l.style.borderRadius="50%",l.style.transform="translate(-50%, -50%)",l.style.pointerEvents="none",l.style.animation="capturePulse 0.4s ease-out",l.style.zIndex="150",l.style.backgroundColor="rgba(76, 175, 80, 0.2)",document.getElementById("camera-container").appendChild(l),setTimeout(()=>{l.remove()},400),Vt(),navigator.vibrate&&navigator.vibrate(50)},500))}),e.addEventListener("touchend",n=>{if(t&&(clearTimeout(t),t=null),!a&&!C&&R.style.display==="none"){va();const i=n.changedTouches[0],s=e.getBoundingClientRect(),o=i.clientX-s.left,r=i.clientY-s.top,l=document.createElement("div");l.style.position="absolute",l.style.left=o+"px",l.style.top=r+"px",l.style.width="60px",l.style.height="60px",l.style.border="2px solid #FE5F00",l.style.borderRadius="50%",l.style.transform="translate(-50%, -50%)",l.style.pointerEvents="none",l.style.animation="focusPulse 0.6s ease-out",l.style.zIndex="150",document.getElementById("camera-container").appendChild(l),setTimeout(()=>{l.remove()},600)}}),e.addEventListener("touchcancel",n=>{t&&(clearTimeout(t),t=null)}),e.addEventListener("click",n=>{if(!C&&R.style.display==="none"){va();const i=e.getBoundingClientRect(),s=n.clientX-i.left,o=n.clientY-i.top,r=document.createElement("div");r.style.position="absolute",r.style.left=s+"px",r.style.top=o+"px",r.style.width="60px",r.style.height="60px",r.style.border="2px solid #FE5F00",r.style.borderRadius="50%",r.style.transform="translate(-50%, -50%)",r.style.pointerEvents="none",r.style.animation="focusPulse 0.6s ease-out",r.style.zIndex="150",document.getElementById("camera-container").appendChild(r),setTimeout(()=>{r.remove()},600)}})}function ia(){const e=document.getElementById("unified-menu");R&&R.style.display==="block"&&na(),we();const t=document.getElementById("styles-count");if(t){const{favorites:a,regular:n}=ft(),i=a.length+n.length;t.textContent=i}Wi(),Yt(),qt(),ut(),C=!0,xe=!0,Le(),Yi(),e.style.display="flex"}async function xa(){C=!1,xe=!1,U=0,Ae="",pe="",document.getElementById("style-filter").value="";const e=document.getElementById("menu-category-hint");e&&(e.style.display="none"),document.getElementById("unified-menu").style.display="none",await qi()}function ue(){const e=document.getElementById("settings-submenu"),t=document.getElementById("unified-menu");Wi(),Yt(),ut(),qt(),t.style.display="none",Le(),e.style.display="flex",C=!1,B=!0,ee=0,setTimeout(()=>{Oa()},50)}function zo(){document.getElementById("settings-submenu").style.display="none",B=!1,ee=0,ia()}function Vo(){const e=document.getElementById("timer-settings-submenu"),t=document.getElementById("settings-submenu"),a=document.getElementById("timer-delay-slider"),n=document.getElementById("timer-delay-value"),i=document.getElementById("timer-repeat-enabled");if(a&&n){const r=pi.indexOf(de);a.value=r!==-1?r+1:3,n.textContent=de}i&&(i.checked=qe);const s=document.getElementById("timer-repeat-interval-input"),o=document.getElementById("timer-repeat-interval-unit");s&&o&&(K>=3600&&K%3600===0?(s.value=K/3600,o.value="3600"):K>=60&&K%60===0?(s.value=K/60,o.value="60"):(s.value=K,o.value="1")),t.style.display="none",Le(),e.style.display="flex",Wt=!0,B=!1}function Yo(){document.getElementById("timer-settings-submenu").style.display="none",Wt=!1,ue()}function qo(){const e=document.querySelector(".styles-menu-scroll-container");e&&(e.scrollTo({top:0,behavior:"smooth"}),U=0,it())}function Ko(){const e=document.querySelector(".styles-menu-scroll-container");if(e){e.scrollTo({top:e.scrollHeight,behavior:"smooth"});const t=document.getElementById("menu-styles-list");if(t){const a=t.querySelectorAll(".style-item");a.length>0&&(U=a.length-1,it())}}}function Wi(){const e=document.getElementById("current-resolution-display");if(e){const t=wt[ye];e.textContent=`${t.width}x${t.height}`}}function Yt(){const e=document.getElementById("current-burst-display");if(e){let t="Medium";for(const[a,n]of Object.entries(Qe))if(n.delay===vt){t=n.label;break}e.textContent=`${j} photos, ${t}`}}function Wo(){document.getElementById("settings-submenu").style.display="none",Le();const e=document.getElementById("resolution-submenu"),t=document.getElementById("resolution-list");t.innerHTML="",wt.forEach((a,n)=>{const i=document.createElement("div");i.className="resolution-item",n===ye&&i.classList.add("active");const s=document.createElement("span");s.className="resolution-name",s.textContent=a.name,i.appendChild(s),i.onclick=()=>{Oo(n),Ji()},t.appendChild(i)}),e.style.display="flex",kt=!0,B=!1,te=0,setTimeout(()=>{const a=e.querySelectorAll(".resolution-item");Sa(a)},100)}async function Ji(){document.getElementById("resolution-submenu").style.display="none",kt=!1,te=0,ue()}function Jo(){document.getElementById("settings-submenu").style.display="none",Le();const e=document.getElementById("burst-submenu"),t=document.getElementById("burst-count-slider"),a=document.getElementById("burst-speed-slider"),n=document.getElementById("burst-count-value"),i=document.getElementById("burst-speed-value");if(t&&n&&(t.value=j,n.textContent=j),a&&i){let s=2;for(const[o,r]of Object.entries(Qe))if(r.delay===vt){s=parseInt(o);break}a.value=s,i.textContent=Qe[s].label}e.style.display="flex",Kt=!0,B=!1}async function _o(){document.getElementById("burst-submenu").style.display="none",Kt=!1,ue()}function $o(){document.getElementById("settings-submenu").style.display="none",Le();const e=document.getElementById("master-prompt-submenu"),t=document.getElementById("master-prompt-enabled"),a=document.getElementById("master-prompt-text"),n=document.getElementById("master-prompt-char-count");t&&(t.checked=ve),a&&(a.value=Z,a.disabled=!ve,n&&(n.textContent=Z.length)),e.style.display="flex",Jt=!0,B=!1}async function Xo(){document.getElementById("master-prompt-submenu").style.display="none",Jt=!1,ue()}function Qo(){document.getElementById("settings-submenu").style.display="none",Le();const e=document.getElementById("aspect-ratio-submenu");e.style.display="flex",B=!1}async function Zo(){document.getElementById("aspect-ratio-submenu").style.display="none",ue()}function li(){const e=document.getElementById("current-aspect-ratio-display");e&&(e.textContent=F==="none"?"None":F)}function qt(){const e=document.getElementById("current-master-prompt-display");if(e)if(ve&&Z.trim()){const t=Z.substring(0,20);e.textContent=`Enabled: ${t}${Z.length>20?"...":""}`}else ve?e.textContent="Enabled (empty)":e.textContent="Disabled"}function xt(){try{localStorage.setItem(yi,Z),localStorage.setItem(bi,ve.toString()),localStorage.setItem(Ti,F)}catch(e){console.error("Failed to save master prompt:",e)}}function er(){try{const e=localStorage.getItem(yi),t=localStorage.getItem(bi);e!==null&&(Z=e),t!==null&&(ve=t==="true");const a=localStorage.getItem(Ti);if(a){F=a;const n=document.getElementById("aspect-ratio-1-1"),i=document.getElementById("aspect-ratio-16-9");n&&(n.checked=F==="1:1"),i&&(i.checked=F==="16:9");const s=document.getElementById("current-aspect-ratio-display");s&&(s.textContent=F==="none"?"None":F)}}catch(e){console.error("Failed to load master prompt:",e)}}function sa(e){let t=e;return ve&&Z.trim()&&(t=`${e} ${Z}`),F==="1:1"?t+=" Use a square aspect ratio.":F==="16:9"&&(t+=" Use a square aspect ratio, but pad the image with black bars at top and bottom to simulate a 16:9 aspect ratio."),t}function we(e=!1){const t=document.getElementById("menu-styles-list");t.innerHTML="",t.replaceWith(t.cloneNode(!1));const a=document.getElementById("menu-styles-list"),n=document.createDocumentFragment(),{favorites:i,regular:s}=ft(),o=i.filter(c=>{if(Ae){const u=Ae.toLowerCase(),f=c.category&&c.category.some(v=>v.toLowerCase().includes(u));if(!(c.name.toLowerCase().includes(u)||c.message.toLowerCase().includes(u)||f))return!1}return pe?c.category&&c.category.includes(pe):!0}),r=s.filter(c=>{if(Ae){const u=Ae.toLowerCase(),f=c.category&&c.category.some(v=>v.toLowerCase().includes(u));if(!(c.name.toLowerCase().includes(u)||c.message.toLowerCase().includes(u)||f))return!1}return pe?c.category&&c.category.includes(pe):!0});if(o.length>0){const c=document.createElement("h3");c.className="menu-section-header",c.textContent="★ Favorites",n.appendChild(c),o.forEach(u=>{const f=ci(u);n.appendChild(f)})}if(r.length>0){const c=document.createElement("h3");c.className="menu-section-header",c.textContent=Ae?"Search Results":"All Styles",n.appendChild(c),r.forEach(u=>{const f=ci(u);n.appendChild(f)})}if(r.length===0&&o.length===0&&Ae){const c=document.createElement("div");c.className="menu-empty",c.textContent="No styles found",n.appendChild(c)}a.appendChild(n),a.addEventListener("click",tr);const l=document.getElementById("styles-count");if(l){const{favorites:c,regular:u}=ft(),f=c.length+u.length;l.textContent=f}e||(U=0,it())}function ci(e){const t=m.findIndex(r=>r===e),a=document.createElement("div");a.className="style-item",a.dataset.index=t,t===T&&a.classList.add("active");const n=document.createElement("button");n.className="style-favorite",n.textContent=at(e.name)?"⭐":"☆",n.dataset.action="favorite",n.dataset.styleName=e.name;const i=document.createElement("span");i.className="style-name",i.textContent=e.name;const s=document.createElement("button");s.className="style-edit";const o=e.internal===!1;return s.textContent=o?"Builder":"Edit",s.dataset.action=o?"builder":"edit",s.dataset.index=t,a.appendChild(n),a.appendChild(i),a.appendChild(s),a}function tr(e){const t=e.target;if(t.dataset.action==="favorite"){e.stopPropagation();const n=t.dataset.styleName;Xs(n);return}if(t.dataset.action==="edit"){e.stopPropagation();const n=parseInt(t.dataset.index);nr(n);return}if(t.dataset.action==="builder"){e.stopPropagation();const n=parseInt(t.dataset.index);so(n);return}const a=t.closest(".style-item");if(a){const n=parseInt(a.dataset.index);isNaN(n)||(T=n,V(),xa())}}function ar(e="Add New Style"){const t=document.getElementById("style-editor");document.getElementById("editor-title").textContent=e,t.style.display="flex"}function La(){document.getElementById("style-editor").style.display="none",document.getElementById("style-name").value="",document.getElementById("style-message").value="";const e=document.getElementById("style-category");e&&(e.value=""),document.getElementById("delete-style").style.display="none",Se=-1}function nr(e){Se=e;const t=m[e];document.getElementById("style-name").value=t.name,document.getElementById("style-message").value=t.message;const a=document.getElementById("style-category");a&&(a.value=t.category?t.category.join(", "):""),document.getElementById("delete-style").style.display="block",ar("Edit Style")}function ir(){const e=document.getElementById("style-name").value.trim(),t=document.getElementById("style-message").value.trim(),a=document.getElementById("style-category").value.trim(),n=a?a.split(",").map(i=>i.trim().toUpperCase()).filter(i=>i.length>0):[];if(!e||!t){alert("Please fill in both name and AI prompt");return}if(Se>=0){const i=m[Se].name;if(m[Se]={name:e,category:n,message:t},i!==e){const s=w.indexOf(i);s>-1&&(w[s]=e,je())}}else m.push({name:e,category:n,message:t}),w.push(e),je();ea(),alert(Se>=0?`Preset "${e}" updated!`:`Preset "${e}" saved!`),La(),ia()}function sr(){Se>=0&&m.length>1&&confirm("Delete this style?")&&(m.splice(Se,1),T>=m.length&&(T=m.length-1),ea(),La(),ia())}function or(){try{const e=new(window.AudioContext||window.webkitAudioContext),t=e.currentTime,a=e.createOscillator(),n=e.createGain(),i=e.createBiquadFilter();a.type="square",a.frequency.setValueAtTime(2400,t),a.frequency.exponentialRampToValueAtTime(1800,t+.012),i.type="highpass",i.frequency.setValueAtTime(1500,t),n.gain.setValueAtTime(.5,t),n.gain.exponentialRampToValueAtTime(.001,t+.015),a.connect(i),i.connect(n),n.connect(e.destination),a.start(t),a.stop(t+.015);const s=e.createOscillator(),o=e.createGain(),r=e.createBiquadFilter();s.type="square",s.frequency.setValueAtTime(1200,t+.015),s.frequency.exponentialRampToValueAtTime(200,t+.023),r.type="bandpass",r.frequency.setValueAtTime(1500,t+.015),r.Q.setValueAtTime(2,t+.015),o.gain.setValueAtTime(.4,t+.015),o.gain.exponentialRampToValueAtTime(.001,t+.03),s.connect(r),r.connect(o),o.connect(e.destination),s.start(t+.015),s.stop(t+.03);const l=e.createOscillator(),c=e.createGain();l.type="triangle",l.frequency.setValueAtTime(400,t+.023),l.frequency.setValueAtTime(450,t+.027),l.frequency.setValueAtTime(380,t+.031),l.frequency.setValueAtTime(420,t+.035),c.gain.setValueAtTime(0,t+.023),c.gain.linearRampToValueAtTime(.15,t+.025),c.gain.exponentialRampToValueAtTime(.001,t+.04),l.connect(c),c.connect(e.destination),l.start(t+.023),l.stop(t+.04);const u=e.createOscillator(),f=e.createGain(),E=e.createBiquadFilter();u.type="square",u.frequency.setValueAtTime(800,t+.05),u.frequency.exponentialRampToValueAtTime(150,t+.06),E.type="bandpass",E.frequency.setValueAtTime(1e3,t+.05),E.Q.setValueAtTime(2,t+.05),f.gain.setValueAtTime(.5,t+.05),f.gain.exponentialRampToValueAtTime(.001,t+.07),u.connect(E),E.connect(f),f.connect(e.destination),u.start(t+.05),u.stop(t+.07);const v=e.createOscillator(),O=e.createGain(),D=e.createBiquadFilter();v.type="sine",v.frequency.setValueAtTime(180,t+.05),v.frequency.exponentialRampToValueAtTime(120,t+.095),D.type="lowpass",D.frequency.setValueAtTime(300,t+.05),O.gain.setValueAtTime(0,t+.05),O.gain.linearRampToValueAtTime(.2,t+.055),O.gain.exponentialRampToValueAtTime(.001,t+.105),v.connect(D),D.connect(O),O.connect(e.destination),v.start(t+.05),v.stop(t+.105);const ie=e.sampleRate*.08,Y=e.createBuffer(1,ie,e.sampleRate),Ee=Y.getChannelData(0);for(let Ue=0;Ue<ie;Ue++){const St=Math.sin(Ue/200)*.5+.5;Ee[Ue]=(Math.random()*2-1)*St}const Pe=e.createBufferSource();Pe.buffer=Y;const Ie=e.createBiquadFilter();Ie.type="bandpass",Ie.frequency.setValueAtTime(3e3,t+.07),Ie.Q.setValueAtTime(1,t+.07);const _=e.createGain();_.gain.setValueAtTime(0,t+.07),_.gain.linearRampToValueAtTime(.12,t+.075),_.gain.linearRampToValueAtTime(.12,t+.125),_.gain.exponentialRampToValueAtTime(.001,t+.15),Pe.connect(Ie),Ie.connect(_),_.connect(e.destination),Pe.start(t+.07),Pe.stop(t+.15);const $=e.createOscillator(),Be=e.createGain();$.type="square",$.frequency.setValueAtTime(600,t+.145),$.frequency.exponentialRampToValueAtTime(100,t+.155),Be.gain.setValueAtTime(.25,t+.145),Be.gain.exponentialRampToValueAtTime(.001,t+.165),$.connect(Be),Be.connect(e.destination),$.start(t+.145),$.stop(t+.165)}catch(e){console.log("Audio generation failed:",e)}}window.addEventListener("load",()=>{Ys(),er(),Go(),Fo();const e=document.getElementById("start-button");e&&e.addEventListener("click",()=>{or();const d=document.querySelector(".camera-body");d&&(d.style.transition="all 0.1s",d.style.boxShadow="0 0 50px rgba(255, 255, 255, 1)",setTimeout(()=>{d.style.boxShadow=""},100));const g=document.querySelector(".lens-inner");g&&(g.style.transition="all 0.05s",g.style.transform="translate(-50%, -50%) scale(0.95)",setTimeout(()=>{g.style.transform="translate(-50%, -50%) scale(1)"},50)),setTimeout(()=>{Bo()},300)});const t=document.getElementById("burst-toggle");t&&t.addEventListener("click",Mo);const a=document.getElementById("timer-toggle");a&&a.addEventListener("click",jo);const n=document.getElementById("random-toggle");n&&n.addEventListener("click",Eo);const i=document.getElementById("motion-toggle");i&&i.addEventListener("click",Zs);const s=document.getElementById("menu-button");s&&s.addEventListener("click",ia);const o=document.getElementById("close-menu");o&&o.addEventListener("click",xa);const r=document.getElementById("jump-to-top");r&&r.addEventListener("click",qo);const l=document.getElementById("jump-to-bottom");l&&l.addEventListener("click",Ko);const c=document.getElementById("settings-menu-button");c&&c.addEventListener("click",ue);const u=document.getElementById("settings-back");u&&u.addEventListener("click",zo);const f=document.getElementById("resolution-settings-button");f&&f.addEventListener("click",Wo);const E=document.getElementById("resolution-back");E&&E.addEventListener("click",Ji);const v=document.getElementById("burst-settings-button");v&&v.addEventListener("click",Jo);const O=document.getElementById("burst-back");O&&O.addEventListener("click",_o);const D=document.getElementById("timer-settings-button");D&&D.addEventListener("click",Vo);const ie=document.getElementById("timer-settings-back");ie&&ie.addEventListener("click",Yo);const Y=document.getElementById("master-prompt-settings-button");Y&&Y.addEventListener("click",$o);const Ee=document.getElementById("master-prompt-back");Ee&&Ee.addEventListener("click",Xo);const Pe=document.getElementById("aspect-ratio-settings-button");Pe&&Pe.addEventListener("click",Qo);const Ie=document.getElementById("aspect-ratio-back");Ie&&Ie.addEventListener("click",Zo);const _=document.getElementById("aspect-ratio-1-1"),$=document.getElementById("aspect-ratio-16-9");_&&_.addEventListener("change",d=>{d.target.checked?(F="1:1",$&&($.checked=!1)):F="none",xt(),li()}),$&&$.addEventListener("change",d=>{d.target.checked?(F="16:9",_&&(_.checked=!1)):F="none",xt(),li()});const Be=document.getElementById("motion-settings-button");Be&&Be.addEventListener("click",to);const Ue=document.getElementById("motion-back");Ue&&Ue.addEventListener("click",ao);const St=document.getElementById("visible-presets-settings-button");St&&St.addEventListener("click",no);const Pa=document.getElementById("visible-presets-back");Pa&&Pa.addEventListener("click",io);const Ba=document.getElementById("preset-builder-button");Ba&&Ba.addEventListener("click",Gi);const Ua=document.getElementById("preset-builder-back");Ua&&Ua.addEventListener("click",Ca);const Da=document.getElementById("preset-builder-jump-up");Da&&Da.addEventListener("click",Mi);const Ha=document.getElementById("preset-builder-jump-down");Ha&&Ha.addEventListener("click",ji);const Ga=document.getElementById("preset-builder-template");Ga&&Ga.addEventListener("change",oo);const Fa=document.getElementById("preset-builder-name");Fa&&Fa.addEventListener("keypress",d=>{var g;d.key==="Enter"&&(d.preventDefault(),(g=document.getElementById("preset-builder-category"))==null||g.focus())});const se=document.getElementById("preset-builder-category"),me=document.getElementById("category-autocomplete");if(se&&me){const d=()=>{const b=se.value,N=b.lastIndexOf(","),q=(N>=0?b.substring(N+1):b).trim().toUpperCase(),Jn=ro(),_n=q?Jn.filter(Ct=>Ct.includes(q)):Jn;_n.length>0?(me.innerHTML=_n.map(Ct=>`<div class="category-autocomplete-item" data-category="${Ct}">${Ct}</div>`).join(""),me.style.display="block"):me.style.display="none"},g=b=>{const N=se.value,q=N.lastIndexOf(",");q>=0?se.value=N.substring(0,q+1)+" "+b+", ":se.value=b+", ",me.style.display="none",se.focus()};se.addEventListener("input",d),se.addEventListener("focus",d),me.addEventListener("click",b=>{if(b.target.classList.contains("category-autocomplete-item")){const N=b.target.getAttribute("data-category");g(N)}}),document.addEventListener("click",b=>{!se.contains(b.target)&&!me.contains(b.target)&&(me.style.display="none")}),se.addEventListener("keypress",b=>{var N;b.key==="Enter"&&(b.preventDefault(),me.style.display="none",(N=document.getElementById("preset-builder-template"))==null||N.focus())})}const za=document.getElementById("preset-builder-save");za&&za.addEventListener("click",lo);const Va=document.getElementById("preset-builder-clear");Va&&Va.addEventListener("click",ta);const Ya=document.getElementById("preset-builder-delete");Ya&&Ya.addEventListener("click",co),document.querySelectorAll(".chip-section-header").forEach(d=>{d.addEventListener("click",()=>{const g=d.getAttribute("data-section"),b=document.getElementById("section-"+g),N=b.style.display==="block";document.querySelectorAll(".chip-section-content").forEach(q=>{q.style.display="none"}),document.querySelectorAll(".chip-section-header").forEach(q=>{q.classList.remove("expanded")}),N||(b.style.display="block",d.classList.add("expanded"))})}),document.querySelectorAll(".preset-chip").forEach(d=>{d.addEventListener("click",g=>{const b=g.target.getAttribute("data-text"),N=document.getElementById("preset-builder-prompt"),q=N.value;q.trim()?N.value=q+" "+b:N.value=b,N.scrollTop=N.scrollHeight})});const qa=document.getElementById("preset-builder-quality");qa&&qa.addEventListener("change",d=>{const g=d.target.value;if(g){const b=document.getElementById("preset-builder-prompt"),N=b.value;N.trim()?b.value=N+" "+g:b.value=g,d.target.value="",b.scrollTop=b.scrollHeight}});const Ka=document.getElementById("visible-presets-filter");Ka&&Ka.addEventListener("input",d=>{pt=d.target.value,$e()});const Wa=document.getElementById("visible-presets-select-all");Wa&&Wa.addEventListener("click",()=>{w=m.filter(g=>!g.internal).map(g=>g.name),je(),$e(),zt(),C&&we()});const Ja=document.getElementById("visible-presets-deselect-all");Ja&&Ja.addEventListener("click",()=>{w=[],je(),$e(),zt(),C&&we()});const _a=document.getElementById("visible-presets-jump-up");_a&&_a.addEventListener("click",()=>{z=0,yt()});const $a=document.getElementById("visible-presets-jump-down");$a&&$a.addEventListener("click",()=>{const d=document.getElementById("visible-presets-list");if(d){const g=d.querySelectorAll(".style-item");g.length>0&&(z=g.length-1,yt())}});const Xa=document.getElementById("white-balance-settings-button");Xa&&Xa.addEventListener("click",_s);const Qa=document.getElementById("white-balance-back");Qa&&Qa.addEventListener("click",Bi);const Za=document.getElementById("motion-sensitivity-slider"),en=document.getElementById("motion-sensitivity-value");if(Za&&en){const d=["Very Low","Low","Medium","High","Very High"];Za.addEventListener("input",g=>{const b=parseInt(g.target.value);en.textContent=d[b-1],Ze=50-b*10,jt(),Fi()})}const tn=document.getElementById("motion-continuous-enabled");tn&&tn.addEventListener("change",d=>{Ve=d.target.checked,jt()});const an=document.getElementById("motion-cooldown-slider"),nn=document.getElementById("motion-cooldown-value");an&&nn&&an.addEventListener("input",d=>{et=parseInt(d.target.value),nn.textContent=`${et}s`,jt()});const sn=document.getElementById("motion-start-delay-slider"),on=document.getElementById("motion-start-delay-value");sn&&on&&sn.addEventListener("input",d=>{const g=parseInt(d.target.value);tt=mt[g].seconds,on.textContent=mt[g].label,jt()});const rn=document.getElementById("no-magic-toggle-button");rn&&rn.addEventListener("click",po);const ln=document.getElementById("tutorial-button");ln&&ln.addEventListener("click",yo);const cn=document.getElementById("tutorial-back");cn&&cn.addEventListener("click",bo),document.querySelectorAll(".glossary-item").forEach(d=>{d.addEventListener("click",()=>{const g=d.getAttribute("data-section");To(g)})});const dn=document.getElementById("back-to-glossary");dn&&dn.addEventListener("click",zi);const hn=document.getElementById("master-prompt-enabled");hn&&hn.addEventListener("change",d=>{ve=d.target.checked;const g=document.getElementById("master-prompt-text");g&&(g.disabled=!ve),xt(),qt()});const un=document.getElementById("master-prompt-text");un&&un.addEventListener("input",d=>{Z=d.target.value;const g=document.getElementById("master-prompt-char-count");g&&(g.textContent=Z.length),xt(),qt()});const mn=document.getElementById("style-filter");let oa=null;mn&&mn.addEventListener("input",d=>{Ae=d.target.value,oa&&clearTimeout(oa),oa=setTimeout(()=>{we()},150)});const gn=document.getElementById("burst-count-slider"),ra=document.getElementById("burst-speed-slider");gn&&gn.addEventListener("input",d=>{j=parseInt(d.target.value),document.getElementById("burst-count-value").textContent=j;const g=parseInt(ra.value);si(j,g),Yt(),Re&&(y.textContent=`Burst mode ON (${j} photos) • ${m[T].name}`)}),ra&&ra.addEventListener("input",d=>{const g=parseInt(d.target.value);vt=Qe[g].delay,document.getElementById("burst-speed-value").textContent=Qe[g].label,si(j,g),Yt()});const pn=document.getElementById("timer-delay-slider"),fn=document.getElementById("timer-delay-value");pn&&fn&&pn.addEventListener("input",d=>{const g=parseInt(d.target.value)-1;de=pi[g],fn.textContent=de,ya(),ut()});const yn=document.getElementById("timer-repeat-enabled");yn&&yn.addEventListener("change",d=>{qe=d.target.checked,ya(),ut()});const la=document.getElementById("timer-repeat-interval-input"),ca=document.getElementById("timer-repeat-interval-unit");if(la&&ca){const d=()=>{const g=parseInt(la.value)||1,b=parseInt(ca.value);K=g*b,ya(),ut()};la.addEventListener("input",d),ca.addEventListener("change",d)}No(),xo(),go(),fo();const bn=document.getElementById("reset-button");bn&&bn.addEventListener("click",na);const Tn=document.getElementById("camera-button");Tn&&Tn.addEventListener("click",Co);const wn=document.getElementById("close-editor");wn&&wn.addEventListener("click",La);const vn=document.getElementById("save-style");vn&&vn.addEventListener("click",ir);const kn=document.getElementById("delete-style");kn&&kn.addEventListener("click",sr),De=document.getElementById("connection-status"),_e=document.getElementById("queue-status"),le=document.getElementById("sync-button"),le&&le.addEventListener("click",Tt),_e&&_e.addEventListener("click",ja);const En=document.getElementById("close-queue-manager");En&&En.addEventListener("click",Uo);const In=document.getElementById("sync-all");In&&In.addEventListener("click",Tt);const An=document.getElementById("clear-queue");An&&An.addEventListener("click",Ki);const Rn=document.getElementById("gallery-button");Rn&&Rn.addEventListener("click",ne);const On=document.getElementById("close-gallery");On&&On.addEventListener("click",ts);const Sn=document.getElementById("close-viewer");Sn&&Sn.addEventListener("click",ss);const Cn=document.getElementById("delete-viewer-image");Cn&&Cn.addEventListener("click",os);const Nn=document.getElementById("gallery-start-date-btn"),da=document.getElementById("gallery-start-date");Nn&&da&&(Nn.addEventListener("click",()=>{da.showPicker()}),da.addEventListener("change",d=>{dt=d.target.value||null,ti("start",dt),pa()}));const Mn=document.getElementById("gallery-end-date-btn"),ha=document.getElementById("gallery-end-date");Mn&&ha&&(Mn.addEventListener("click",()=>{ha.showPicker()}),ha.addEventListener("change",d=>{ht=d.target.value||null,ti("end",ht),pa()}));const jn=document.getElementById("gallery-sort-order");jn&&jn.addEventListener("change",d=>{gt=d.target.value;try{localStorage.setItem(ki,gt)}catch(g){console.error("Failed to save sort order:",g)}pa()});const xn=document.getElementById("prev-page");xn&&xn.addEventListener("click",ns);const Ln=document.getElementById("next-page");Ln&&Ln.addEventListener("click",as);const Pn=document.getElementById("load-preset-button");Pn&&Pn.addEventListener("click",rs);const Bn=document.getElementById("multi-preset-button");Bn&&Bn.addEventListener("click",()=>{if(J>=0){const d=M[J].id;Ds(d)}});const Un=document.getElementById("close-preset-selector");Un&&Un.addEventListener("click",Gt);const Dn=document.getElementById("preset-filter");Dn&&Dn.addEventListener("input",d=>{Dt=d.target.value,It()});const Hn=document.getElementById("preset-selector-jump-up");Hn&&Hn.addEventListener("click",()=>{L=0,Ye()});const Gn=document.getElementById("preset-selector-jump-down");Gn&&Gn.addEventListener("click",()=>{const d=document.getElementById("preset-list");if(d){const g=d.querySelectorAll(".preset-item");g.length>0&&(L=g.length-1,Ye())}});const Fn=document.getElementById("magic-button");Fn&&Fn.addEventListener("click",js);const zn=document.getElementById("batch-mode-toggle");zn&&zn.addEventListener("click",Ft);const Vn=document.getElementById("batch-select-all");Vn&&Vn.addEventListener("click",xs);const Yn=document.getElementById("batch-deselect-all");Yn&&Yn.addEventListener("click",Ls);const qn=document.getElementById("batch-cancel");qn&&qn.addEventListener("click",Ft);const Kn=document.getElementById("batch-apply-preset");Kn&&Kn.addEventListener("click",Ps);const Wn=document.getElementById("batch-delete");Wn&&Wn.addEventListener("click",Us),Xt().then(async()=>{localStorage.getItem("r1_gallery_index")?(console.log("Migrating old gallery data..."),await Qi()):await Qt()}).catch(d=>{console.error("Failed to initialize database:",d)}),Gs()});window.removeFromQueue=Do;window.previewQueueItem=Ho;window.clearQueue=Ki;console.log("AI Camera Styles app initialized!");
